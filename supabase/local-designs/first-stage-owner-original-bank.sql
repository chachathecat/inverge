-- UNAPPROVED INSTALLATION PROPOSAL. DO NOT APPLY TO ANY PERSONAL/REMOTE DB.
-- Prepared only for inverge_owner_economics_db_loopback / postgres after Owner approval,
-- exact SQL review, restorable backup and isolated verification. No approval is implied.
-- Body reused unchanged from PR #959 source 61c10444 / main a46ede00.
-- Rollback disables the local bundle flag; preserve all rows and additive objects.
begin;
set local lock_timeout = '5s';
set local statement_timeout = '10s';
do $$ begin
  if current_setting('inverge.owner_pc_original_content_install',true) is distinct from 'owner_approved_exact_original_bundle'
    or current_setting('inverge.local_first_stage_personal_use',true) is distinct from 'owner_approved_persistent_local'
    or nullif(current_setting('inverge.local_first_stage_design',true),'') is not null then
    raise exception 'explicit persistent owner-local original bundle approval required';
  end if;
end $$;
