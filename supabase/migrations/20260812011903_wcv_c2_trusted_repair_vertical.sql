-- WCV-C2 first trusted repair vertical.
-- Forward-only rollback: disable WCV_C2_TRUSTED_REPAIR_ENABLED first, then
-- revert this migration in a fresh isolated environment. No remote application
-- is authorized by the 2026-08-12 Owner decision.
...
create table if not exists public.wcv_c2_trusted_repair_sessions (
  ...
  validator_version text not null check (
    validator_version = 'wcv_c2_deterministic_subject_oracles.v1'
  ),
...
comment on table public.wcv_c2_trusted_repair_sessions is
  'Bodyless canonical WCV-C2 session metadata and exact version bindings.';
