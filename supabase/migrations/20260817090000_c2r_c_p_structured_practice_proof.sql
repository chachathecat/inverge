...
create table if not exists public.wcv_c2_trusted_repair_sessions (
  ...
  contract_version text not null check (
    contract_version = 'wcv_c2r_c_p_structured_practice_proof.v2'
  ),
  ...
  validator_version text not null check (
    validator_version = 'validator:practice-calculation-claim@2'
  ),
...
begin
  ...
  if p_next_state = 'verified' and (
    ...
    raise exception 'WCV_C2_STRUCTURED_PROOF_REQUIRED';
  end if;
...
comment on function public.wcv_c2_apply_trusted_repair_transition_v1 is
  'Service-only exact-user CAS transition with atomic private-artifact/exposure/structured-proof state commit and replay receipt.';
