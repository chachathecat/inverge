...
## First-input 60초 점검 (신규)

- [ ] 신규 사용자(데이터 없음) 1차 진입 시 입력부터 시작한다. (`/app/capture?mode=first`)
- [ ] 신규 사용자(데이터 없음) 2차 진입 시 입력부터 시작한다. (`/app/write?mode=second`)
- [ ] 기존 사용자(의미 있는 데이터 있음) 1차/2차 진입 시 오늘부터 시작한다. (`/app?mode=...`)
- [ ] 데이터 없는 사용자가 상단 `오늘`을 직접 눌러도 입력 안내 상태가 차분하게 보인다. (루프/강제 리다이렉트 없음)
- [ ] 답안 검토 텍스트-only 경로로 Step 1을 완료할 수 있는지 확인
- [ ] 첫 저장 직후 `방금 남긴 입력은 오늘 할 일과 노트에 반영됩니다.` 문구가 보이는지 확인
- [ ] 텍스트 입력 후 다음 행동 버튼이 화면에 명확히 보인다.
- [ ] 1차 입력에서 disabled 저장 버튼 때문에 사용자가 멈추지 않는다.
- [ ] 2차 입력에서 이미 쓴 답안 검토와 새 답안 작성이 구분된다.
...
### 5.4 상단 내비 단순성
- [ ] 상단 내비가 `입력 / 오늘 / 노트` 중심으로 단순하게 유지되는지 확인
- 기록:
  - Status:
  - Notes:
  - Screenshot (optional):
...
### 5.5 노트 화면의 비-대시보드 톤
- [ ] 노트 페이지가 복잡한 관제 대시보드처럼 느껴지지 않는지 확인
- 기록:
  - Status:
  - Notes:
  - Screenshot (optional):
...
- [ ] Today Plan/노트 화면이 누적 가치(최근 행동/반복 신호/다시 볼 항목)를 보여주는지 확인
...
- [ ] 첫 입력 직후 사용자가 오늘 할 일 또는 노트에서 반영 위치를 바로 찾을 수 있다.
- [ ] 저장 완료 CTA 문구와 목적지가 일치한다. (`노트에서 확인` → `/app/items`, `오늘 확인` → `/app`, `다시 볼 항목 확인` → `/app/review`)
