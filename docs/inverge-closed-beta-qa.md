...
## Authenticated Closed Beta E2E QA — PR #294

- Branch basis: latest checked-out main after PR #293 (`5a58d6e`, `Polish first O/X review detail precision (#293)`). The repository had no configured `origin` remote in this runtime, so the branch could not fetch a newer remote main from here.
...
- Tested base URL: **not verifiable** — `E2E_BASE_URL` was missing. No host, credentials, account emails, screenshots, cookies, or browser artifacts were recorded.
- Secret-handling result: no credential values were printed, committed, documented, or captured. The environment check printed only `set` / `missing` status for the five allowed variable names.
...
- Release-blocker result: no new P0/P1 blocker was found by learner-loop, closed-beta-readiness, taxonomy, lint, or production-build gates. Authenticated browser QA remains manual-only until the requested secret E2E variables are present in the runner.

### Commands run on 2026-06-01

| Command | Result | Evidence / notes |
| --- | --- | --- |
| `npm run verify:learner-loop:ci` | Pass | 152 learner-loop tests passed, 5 quality-eval tests passed, taxonomy check passed, and production build completed. Build emitted the existing non-fatal Turbopack NFT tracing warning for `next.config.ts` → `lib/review-os/question-reference.ts` → `app/app/items/[itemId]/page.tsx`. |
| `npm run check:closed-beta-readiness` | Pass | Learner-loop verification, data-boundary tests, question-reference tests, route/source guard checks, and production build passed. Final script output: `[closed-beta-readiness] PASS: closed beta learner loop, data boundary, route/source guards, question references, and build passed.` |
...
| `npm run build` | Pass | Next.js production build completed, TypeScript passed, and 46 static pages generated. The same Turbopack NFT tracing warning remained non-fatal. |
| `node - <<'NODE' ... NODE` | Not verifiable | Checked only whether `E2E_BASE_URL`, `E2E_USER_EMAIL`, `E2E_USER_PASSWORD`, `E2E_NOT_INVITED_EMAIL`, and `E2E_NOT_INVITED_PASSWORD` were present. All five were missing, so authenticated browser flows could not run without violating the missing-secret rule. |
| `npm run lint` | Pass with existing warnings | ESLint exited 0 with 10 pre-existing unused-variable warnings and no errors. |

### Required authenticated flow evidence

| Flow | Result | Route / surface | Evidence / notes | Remaining risk category |
| --- | --- | --- | --- | --- |
| 1. Non-invited account access | Not verifiable | `/login`, `/app` | `E2E_BASE_URL`, `E2E_NOT_INVITED_EMAIL`, and `E2E_NOT_INVITED_PASSWORD` were missing. Source/readiness guards still verify invite-only learner access and blocked invite-state coverage for “아직 초대 승인 전입니다.” | Manual-only: log in with the non-invited account, open `/app`, confirm learner data is inaccessible and the blocked invite state is calm. |
| 2. Invited account access | Not verifiable | `/login`, `/app` | `E2E_BASE_URL`, `E2E_USER_EMAIL`, and `E2E_USER_PASSWORD` were missing. Readiness guards verify learner navigation excludes `/admin`, `/instructor`, `/studio`, `/pricing`, `/checkout`, and `/exams/archive`, but live invited-account browser access could not be checked. | Manual-only: log in with the invited account, confirm `/app` loads the learner operating screen and learner nav does not expose restricted destinations. |
| 3. `/exams` product scope | Source/script pass; authenticated browser not verifiable | `/exams` | Automated route/source guards verify the exam-selection surface shows only `감정평가사 1차` and `감정평가사 2차`, and does not expose 답안 검토실, 운영자용 베타, 보험계리사, CPA, 세무사, TOEFL, SAT, or public archive UI. Live hosted route verification was blocked because `E2E_BASE_URL` was missing. | Manual-only: open the hosted `/exams` page and confirm the same learner-facing scope. |
| 4. Capture-to-Note | Source/script pass; authenticated browser not verifiable | `/app/capture` | Learner-loop/readiness checks verify photo/PDF/text entry coverage, editable OCR draft behavior, draft-not-final-truth copy, safe save metadata, and no official grading/final score/pass-fail/model-answer claims. Browser route access with the invited account was not verifiable without the base URL and credentials. | Manual-only: real mobile camera capture and actual handwritten OCR quality. |
| 5. Today Plan / Review Queue | Source/script pass; authenticated browser not verifiable | `/app`, `/app/review` | Automated checks verify Today Plan appears in the learner shell, primary tasks are capped at 3, details/reference hints are collapsed by default, score/pass/fail prediction language is absent, and review surfaces avoid E2E/smoke/test seed data exposure in source-level guards. Live account data could not be checked without the E2E secrets. | Manual-only: confirm the invited account’s live Today Plan/review queue contents and mobile reading comfort. |
| 6. 1차 O/X | Source/script pass; authenticated browser not verifiable | `/app/first/ox` | Learner-loop/readiness checks verify one statement at a time, the five answer buttons (`O + 확실함`, `X + 확실함`, `O + 헷갈림`, `X + 헷갈림`, `모름`), gated concept popup after answering, safe review-signal behavior for 모름/헷갈림, and no correct+certain queue flood. Live click-through could not run without `E2E_BASE_URL` and invited credentials. | Manual-only: click 모름/헷갈림 in the hosted app and confirm saved/review status. |
| 7. O/X saved-statement retry | Source/script pass; authenticated saved-item retry not verifiable | `/app/first/ox?retryItemId=...`, `/app/items/[itemId]` | Tests verify detail-item retry routing, the copy “저장된 선지를 다시 판단합니다.”, and loading user-owned saved raw statements instead of only built-in samples. Account-backed item creation/retry could not be verified because the E2E base URL and credentials were missing. | Manual-only: create or open a saved O/X item, click “같은 선지 다시 판단하기.”, and confirm the retry URL and original statement. |
| 8. 2차 Rewrite / CASIO | Source/script pass; authenticated browser not verifiable | `/app/write`, `/app/calculator`, `/app/items/[itemId]` | Learner-loop/readiness checks verify one biggest gap plus one rewrite action, deterministic CASIO keystrokes only for supported templates, unsupported fallback copy “타건 순서는 지원되는 계산 템플릿에 한해 제공됩니다.”, and no official score/model-answer/pass-fail/final-judgment copy. Live hosted browser verification was blocked by missing E2E variables. | Manual-only: complete the invited-account second-mode flow and confirm CASIO/rewrite copy in the live app. |
| 9. Data boundary and reference safety | Pass | Automated data-boundary/reference tests | `npm run check:closed-beta-readiness` reran data-boundary and question-reference tests. The gates verify raw OCR, user answer, uploaded problem text, rewrite paragraph, and handwritten-content-equivalent raw fields are stripped from derived metadata, telemetry, reference requests, and question reference matching; question archive records remain metadata-only and no public archive UI is introduced. | P2/manual-only: final hosted production environment check after secrets are available. |

### Unresolved issues and blocker classification

- P0/P1: none found by the automated gates that could run in this environment.
- P2/manual-only: authenticated invited and non-invited browser QA could not be completed because all requested E2E variables were missing.
- Manual-only: real mobile camera capture.
- Manual-only: actual handwritten OCR quality.
- Manual-only: final production environment check.
- Manual-only: real user mobile touch/reading comfort.
