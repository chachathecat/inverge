...
- Completed: representative Theory/Practice repair loops; bounded synthetic Law controlled save/reconnect/paragraph review/Today. Preserve September 18/19 schedules and the disabled Law account.
- Existing connections to validate: imported/corrected page text into separate problem/answer, source-only storage, authenticated resume and the existing analysis/repair path. Fix the first observed break in this same candidate: merged page input has no separation action, and existing-answer preparation lacks the required imported-source confirmation control.
- Additional implementation: explicit learner-selected text movement, retained source text and confirmation invalidation on edits. No inference of an answer, automatic provider call or new input system.
- Content supply shortage: the existing Law registry has ten unverified candidates with open blockers and no applicable-date/evidence package. Its independent snapshot reader proves integrity only; currentness and applicability remain unverified. Keep the synthetic Article 10 gate. Continue existing five-subject loader/Foundry supply and schedule connections after this input outcome; no new bank or mastery ledger.
- Approval pending: actual Law source-to-learner/provider operation and paid model calls. Preserve Theory 7/7, Practice 5/5, USD 4.388969 reservations, USD 0.611031 remaining and all unknown/unmeasured usage. Theory paraphrase and general Law accuracy remain unverified. Photos/PDF manual input is distinct from actual OCR quality. Use the existing active isolated account with normal auth/quotas and no provider key; do not create or reactivate accounts.

Dedicated issue #952. Actual Next.js normal-menu photo and two-page PDF manual-input runs now prove source preservation, explicit separation/confirmation, save and authenticated reconnect. SQL confirms both sources remain not-analyzed, with no generated notes or learning-record changes. The runs also exposed and fixed selected-subject loss during import and the non-persisted existing-answer entry choice. PR #953 also corrects imported-source confirmation across the existing answer editor, source editor and page reorder/removal; pending confirmation survives reload. Final source head f6305976 passed both real-app photo/PDF save/reconnect/API/SQL runs and independent actionable P0/P1/P2 0/0/0 review. Seventeen focused and thirty-five related tests pass; the initial full suite passed 1976/1977, with the unchanged budget-concurrency suite passing 5/5 in isolation after its load-sensitive failure. Final typecheck and lint pass (one existing warning). Native final-head checks remain required; actual OCR and model quality are not claimed. Personal fourteen-table and thirty-two configuration/budget-file preservation checks pass. No personal/remote DB, source collection, provider, activation or Production changes. Final native checks, independent review and protected squash remain required.


## 2026-09-17 Practice continuation

Continue the same PC writer after merged #946 / 763ee1b5. Preserve the personal app, Theory artifacts and September 18 review. Reuse the isolated account/DB and ordinary Next.js menus for a single original direct-capitalization problem. First repair the observed subject entry and calculation/correction connection; compare amounts, units and rounding with an independent integer oracle. Controlled external-provider responses are free integration evidence only: authentication, application routes and persistence stay real. Existing Theory development calls remain 7/7 and the shared USD 5 ledger is unchanged. Paid Practice verification is blocked pending one separate bounded Owner approval. No personal schema, remote DB, Production, public activation or legal-collection changes. Apply the existing post909 A/B/C exception and retain exact-head checks, independent review and protected squash.
...
- PR #882 is merged at `7626b82e8ebc9649ae2da125077900458e762b21`, tree
  `a0b82d209f3aa6c28e06b11e77f4a21c617ba5ef`; the tree exactly matches the
  approved candidate. Native evidence and actual post-merge build cancellation:
  https://github.com/chachathecat/inverge/pull/882#issuecomment-5558716000.
- Current lead: #883, the representative private economics question/response/
  explanation/save/reconnect/review slice. Five-subject completion is not claimed.
- Single writer: `codex/appraiser-first-private-learner-loop`, using the existing
  isolated `pr882-appraiser-first-rescope` worktree. This checkpoint accompanies
  the first successor candidate; live GitHub remains authority for its later
  commit, PR, required checks, exact-head review and merge status.
- Implemented draft: unchanged common MCQ kernel plus trusted-clock service,
  deterministic replay/CAS, durable-read-before-explanation projection, bounded
  16-KiB Request/Response handler and existing-Supabase-transport repository.
  `supabase/local-designs/first-stage-private-sessions.sql` is isolated local
  design only, not a migration or remote apply authority. It requires an explicit
  synthetic-only context and denies client writes; existing policies are untouched.
- Prior evidence (before the real content loader): 39 affected kernel/capacity/service/actual-route tests passed, plus
  one real PostgreSQL/browser acceptance (40 total). The new route source and
  server composition execute with injected auth/catalog/clock ports; the actual
  SDK/repository uses network-none PostgreSQL. Concurrent create, committed-write
  response loss, reconnect, exact D+1, completion-preserving replay and null/owner
  drift rejection pass. The actual client component completed response/save/
  retry/reload/review through the route in a loopback browser host, with zero
  external browser requests/errors and zero browser-storage records. Synthetic
  users/rows were cleaned to zero and the disposable container removed.
  This is NOT remote authentication, deployed Next RSC or real-content acceptance.
- Connected `/app/first-stage/practice` and `/api/review-os/first-stage/sessions`
  behind the existing Owner/default-off/non-Production criteria. Initial page
  props contain no content. No approved loader stock exists: availability is
  explicitly blocked and storage is not opened without approved content.
- Current typecheck, changed-file lint and build passed. Build reported an
  NFT broad-tracing warning in the unchanged curriculum-reference dependency
  chain. Frozen Kernel and Study Capacity suites passed without changing them.
- The first wrapper submission shape and then its oversized parser envelope
  failed new tests; both were corrected at the new wrapper boundary, preserving
  the kernel's clock-sealing parser and 20,000-character limit. QF's 64-KiB limit
  is untouched.
- Owner's 2026-09-06 follow-up explicitly includes initial content preparation
  inside #883: separately verify the specified Q-Net problem/final-key posts,
  prepare five economics originals and five independently solved retry candidates,
  and present one human-review packet. AI/code checks never count as human review.
  Do not request a pre-completed pack path again or stop independent code work.
- Q-Net posts 5231525 and 5246129 each currently display KOGL Type 1. Downloaded
  exact attachments 2230215 (1교시 PDF, 36 pages) and 2243629 (final-key HWP) to
  `C:/Users/jmg91/.cache/codex-runtimes/owner-private/issue-883-economics-2025`.
  Bodies remain outside Git/CI/logs. Private `review-packet.html`, JSON and
  `REVIEW.md` now exist with five originals and five separately solved modified
  retry candidates. `calculation-results.json` records 10/10 code checks; source
  PDF/HWP SHA-256 identities match. Each post's raw HTML and KOGL terms are saved
  as non-executable private text. No existing development candidate is stock.
  A-form appears in the problem PDF but no explicit booklet label was found in
  the final-key HWP; attachment-license applicability and economics judgments
  remain human-review items. No human reviewer or approval has been invented.
- Human review requested once as a concrete six-item checklist with the actual
  private packet, not another request for a pre-completed pack path. Independent
  code work has continued. Functional tests do not approve content.
- New implementation: bounded server-only private content loader and real
  economics SubjectAdapterV1, exact separately installed approval SHA/version,
  reference/choice/key matching, independent practice retry keys, finite
  deterministic stock and explicit exhausted-stock UI. Installed approvals are
  empty. Runtime rejects test receipts and client approval claims. The private
  review packet is not a runtime catalog. The frozen kernel/interface is unchanged.
- New execution: actual loader/adapter runs inside the real route/service/
  repository/browser acceptance with synthetic content and auth/clock ports.
  Missing approval, version/key/lineage drift, wrong answer authority, pre-save
  disclosure, concurrent create/submit/retry, partial-save response loss,
...
- Prior packet evidence: r2 JSON SHA-256
  ...
  marginal-cost wording and the Q derivative were corrected. Ten independent
  AI/code calculations pass and bind the exact corrected packet and calculator.
  All 10 rows/50 content fields were audited with zero cross-row copies; the
  three saved post/license HTML hashes and two original source file hashes match.
  Prior r1 is preserved privately. REVIEW.md keeps the six human checks pending
  and records the additional AI findings separately, with no invented reviewer.
...
- No runtime flag, Production/public service, remote DB, provider, payment or
  real learner data was changed. No question/reference body is added to Git.

| Subject | Functional support in this work | Actual approved content |
| --- | --- | --- |
| Economics | Common-kernel private session mechanics tested; reviewed loader/adapter pending | 0; 5+5 private review candidates only |
| Civil law | Existing frozen common interface only; no subject implementation here yet | 0 |
| Real-estate principles | Existing frozen common interface only; no subject implementation here yet | 0 |
| Appraisal-related law | Existing frozen common interface only; no subject implementation here yet | 0 |
| Accounting | Existing frozen common interface only; development candidates not approved | 0 |

Rollback: leave the existing feature flag OFF, remove the new private route/UI
link with an ordinary reviewed follow-up if necessary, and preserve durable
records. The local SQL design has not entered migration history or a remote
database. No destructive database rollback or environment mutation is authorized.

The sections below are the historical pre-PR-845 checkpoint, not the current
work selector or current merge authority. Live GitHub remains authoritative.
...
### 2026-09-17 Practice paid validation amendment

Owner approved at most four Practice development calls (weak analysis, correction confirmation, correct-answer comparison, optional retry) for the one original direct-capitalization fixture, with at most three synthetic answers. Project/model and token caps remain unchanged; failures/UNKNOWN consume the cap. Additional maximum reservation is USD 1.350452 within the original shared USD 5 ledger, preserving Theory 7/7 and personal binding. No automatic retry or personal/Production/database activation. Dedicated issue #948; required checks, exact-head independent review, protected squash and post909 A/B/C correction exception remain applicable.

### 2026-09-17 Practice fifth-call amendment

Owner raised the existing Practice total from four to five, including the three calls already used. The two remaining calls are exclusively fresh analysis and correction confirmation for the same synthetic fixture, immediately followed by real save. One append-only extension binds the unchanged original approval; added maximum reservation is USD 0.337613 within the same USD 5 ledger, with Theory 7/7, all UNKNOWN/unmeasured reservations and personal binding preserved. Finish code, save regressions and final runtime preparation before the paid pair; do not restart, edit, wait for CI or write reports between analysis, confirmation and save. Expiry remains unchanged. Merge and PC code-only update still require actual paid-source reconnect/review/Today acceptance and all final-head checks/review.

### 2026-09-17 Practice completion and Law continuation

PR #949 merged at 23c75314 after final ec4679f8 native checks/review and actual Gemini analysis → correction confirmation → durable save → reconnect → Practice review → Learning Record/Today. Personal PC code is updated with existing flags/data preserved; Theory September 18 review remains pending. Practice is 5/5, Theory 7/7, permanent reservations USD 4.388969 and remaining USD 0.611031. No Law paid approval.

Consumed the existing acceleration instruction after Practice acceptance. Same PC writer continues on codex/law-real-app-continuation for dedicated issue #950, leaving #772/#883 open. Reuse the existing synthetic Article 10 applicability fixture, normal Capture/APP1 storage and Review/Today; real-law currentness/subsumption remain unsupported. Add server source/version/date checks at analysis/confirmation/save, preserve typed correction fields, and fix private paragraph review persistence. No schema, personal activation, collection or provider changes.

Focused regressions: 72 pass; typecheck and optimized Next build pass. Actual isolated normal-menu Capture → Law selection → existing-answer entry → real authenticated SQL input save passes. Analysis is honestly blocked by the existing account's normal 402 daily quota. An entitlement-bypass proposal was automatically rejected and not applied. A separate explicit request is pending for one isolated synthetic account with unchanged normal checks; otherwise wait for normal quota renewal. Actual model, complete Law browser save/review, native final-head checks and independent final review remain pending; do not merge or claim full Law acceptance.

Owner now approved exactly one additional isolated Core account, with ordinary auth/quota checks and exactly two durable-counted controlled responses (analysis and correction), then account deactivation with evidence retained. No paid key/model calls or quota reset. Independent review identified and corrected the Today missing self-rating and Law negation consistency defects; behavioral regressions cover both. Repository persistence/access code and its frozen security test remain byte-identical to main: the bounded Law notice reuses the existing comparisonPoint field. Actual full flow and exact-head review/checks are still required.
