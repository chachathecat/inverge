...
- `target`: `auto`, a roadmap item id such as `S215`, a PR number such as `461`, or a sanitized fixture path.
...
S214 completion makes S215 the next queued/ready example target for plan-only dispatch examples until the active roadmap advances again.
