...
- `target`: `auto`, a roadmap item id such as `S214`, a PR number such as `461`, or a sanitized fixture path.
...
S213 completion makes S214 the next queued/ready example target for plan-only dispatch examples until the active roadmap advances again.
