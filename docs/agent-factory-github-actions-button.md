...
- `target`: `auto`, a roadmap item id such as `S220`, a PR number such as `461`, or a sanitized fixture path.
...
S219 completion makes S220 the next queued/ready example target for plan-only dispatch examples until the active roadmap advances again.
