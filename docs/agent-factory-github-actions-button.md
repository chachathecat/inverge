...
- `target`: `auto`, a roadmap item id such as `S223`, a PR number such as `461`, or a sanitized fixture path.
...
S222 completion makes S223 the next queued/ready example target for plan-only dispatch examples until the active roadmap advances again.
