...
- `target`: `auto`, a roadmap item id such as `S216`, a PR number such as `461`, or a sanitized fixture path.
...
S215 completion makes S216 the next queued/ready example target for plan-only dispatch examples until the active roadmap advances again.
