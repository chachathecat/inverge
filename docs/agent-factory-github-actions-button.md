...
- `target`: `auto`, a roadmap item id such as `S225` (a blocked example) or
  `S236B` (a queued report-only example), a PR number such as `461`,
...
cross-surface canary is incomplete. The only ready item is S236B, but Draft
PR #660 is nonqualifying and explicitly forbids automatic additional OCR
work. Automatic plan-only selection reports S236B; that report is not start
authority. Completed O3A/O4V targets and blocked S225/S236P targets fail closed.
