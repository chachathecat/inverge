...
- `target`: `auto`, a roadmap item id such as `S224`, a PR number such as `461`, or a sanitized fixture path.
...
S223 completion makes S224 the next queued/ready example target for plan-only dispatch examples until the active roadmap advances again.
