...
- `target`: `auto`, a roadmap item id such as `S222`, a PR number such as `461`, or a sanitized fixture path.
...
S221 completion makes S222 the next queued/ready example target for plan-only dispatch examples until the active roadmap advances again.
