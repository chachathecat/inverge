...
- `target`: `auto`, a roadmap item id such as `S221`, a PR number such as `461`, or a sanitized fixture path.
...
S220 completion makes S221 the next queued/ready example target for plan-only dispatch examples until the active roadmap advances again.
