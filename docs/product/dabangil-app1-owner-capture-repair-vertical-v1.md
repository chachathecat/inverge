...
After a durable Capture save, the gated path continues to `/app/capture/repair?itemId=<id>` only when the persisted learner answer satisfies the same bounded Answer Review input-quality prerequisites used by APP-1 initial analysis and the persisted OCR/source state is eligible. Photo, image and PDF captures require explicit OCR confirmation; any recorded low-confidence state also requires a persisted manual correction. A saved issue recall, outline, placeholder, too-short answer or unresolved OCR/source capture remains on the ordinary durable saved-plan continuation instead of entering a repair page that cannot analyze or authorize it. The repair route reloads the learner-owned record and shows a bounded structure summary: subject, detected sections, page or section count, OCR-confirmation state and any known uncertainty. The Owner chooses `이 내용으로 분석` only after confirming the material.
...
- Any recorded OCR or source uncertainty blocks confirmation even when the learner acknowledged the extraction; acknowledgement alone is not resolution. A persisted learner correction of the uncertain source text is required before a new analysis may proceed.
