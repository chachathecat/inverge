...
After a durable Capture save, the gated path continues to `/app/capture/repair?itemId=<id>` only when the persisted learner answer satisfies the same bounded Answer Review input-quality prerequisites used by APP-1 initial analysis. A saved issue recall, outline, placeholder or too-short answer remains on the ordinary durable saved-plan continuation instead of entering a repair page that cannot analyze it. The repair route reloads the learner-owned record and shows a bounded structure summary: subject, detected sections, page or section count, OCR-confirmation state and any known uncertainty. The Owner chooses `이 내용으로 분석` only after confirming the material.
