...
Terminal PRs #790, #791, #792, and #793 are closed-unmerged read-only donors only. The replacement starts from refreshed main without donor ancestry. Their historical findings are reproduced as hostile fixtures; their prose, red checks, and artifacts are not replacement authority. No governed issue is closed by this Draft.

## Exact PostgreSQL source and version binding

This oracle is bound only to PostgreSQL tag `REL_15_8`, source commit `3cc5e51ab4507b33acc82684b7d79ac43b8a6b5d`, `server_version_num = 150008`, and image digest `eb3747f5d0a92195ca486d2f15d9a4ee5e9461b0332fe87fbc59069490a5c659`. The observed behavior is version-bound and must not be generalized to another PostgreSQL release.

In the exact `REL_15_8` grammar, role-membership GRANT assigns `opt_granted_by` to `stmt->grantor`. Role-membership REVOKE parses `opt_granted_by` but does not assign it to `stmt->grantor`. Execution therefore resolves the non-null named GRANT grantor, so the nonexistent role rejects with SQLSTATE `42704` before mutation. The exact REVOKE path reaches membership deletion without resolving the parsed nonexistent grantor. Only the REVOKE `GRANTED BY` clause is ignored on this PostgreSQL-15.8 path; the REVOKE operation succeeds and removes the exact membership edge. This is not an authorization bypass: membership deletion still checks the current session user, and ordinal 22 executes as `postgres`; only the parsed grantor selector is discarded.

Exact server execution and pre/post catalog state are authoritative for this pinned profile. Ordinal 21 proves the GRANT rejection and unchanged semantic state. Ordinal 22 first asserts that the exact target edge exists, executes the version-bound REVOKE without a SQLSTATE diagnostic, returns membership count `0`, and proves exactly one removal of `oracle_grantor` → `oracle_direct` with every other semantic catalog collection unchanged.
...
Every expected SQL rejection is one exact statement. The oracle requires `psql` script-error status 3, a fixture-allowlisted five-character SQLSTATE, identical pre/post collection and root digests, and an exact empty row-level delta. Warnings or infrastructure failures cannot satisfy a rejection receipt. Every successful fixture requires status 0 and the SHA-256 of its exact closed normalized stdout bytes, including the empty-output expectation. The version-bound ordinal-22 success additionally requires the exact membership-only removal counts, membership delta digest, whole delta root, and zero delta in all other collections.
...
- superuser, CREATEROLE, ADMIN OPTION, duplicate plain GRANT, `REVOKE ADMIN OPTION FOR`, unauthorized GRANT/REVOKE, and the exact PostgreSQL-15.8 asymmetry where nonexistent `GRANTED BY` rejects GRANT with `42704` but its clause is ignored by role-membership REVOKE while the membership is removed;
