...
5. #770 and #780 remain terminal read-only donors;
...
8. `UNKNOWN` and `KNOWN_APPLIED` states block silent rename;
9. remote migration mutation requires a separate Owner gate;
10. runtime-adapter path closure and both reset cycles are mandatory;
11. one merge-producing writer remains enforced;
12. Production, payment and external learner remain unauthorized;
13. V13 remains the sole active master; and
14. no successor runtime is started by this source-only Work.
