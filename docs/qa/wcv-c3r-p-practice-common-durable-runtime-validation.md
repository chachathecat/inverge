...
The failure was a runtime-harness substrate mismatch, not authority drift or a defect that this stage may repair in S236P: the inherited local Supabase config disabled Storage, the donor runner excluded `storage-api`, and then it replayed a history whose A0 registry correctly declares `storage.buckets`, `storage.objects`, `storage.allow_only_operation(text)` and `storage.allow_any_operation(text[])` as external database objects. The existing generic Runtime Gate succeeded because its isolated fixture establishes those external objects before applying S236P.

PR #799's two bounded corrections confirmed that a runner-owned imitation is not a valid substitute for the external Supabase Storage provider: the final adapter reached the initialized Storage tables but failed when it attempted an ownership-restricted `alter table storage.objects`. The first PR #800 head then exposed the remaining harness-only mismatch: six preflight booleans were explicitly cast to full `true`/`false` text while the final `relrowsecurity` catalog boolean was left uncast, so its compact PostgreSQL text form could not equal the closed seven-value string. After that normalization passed and the exact history replay completed, the runner supplied the absolute browser spec path as a Playwright positional filter even though the pinned config already selected that exact spec; Playwright therefore exited with `No tests found` before auth or application execution. This clean replan enables the isolated local Storage service, keeps `storage-api` in the started service set, waits for Supabase startup health, explicitly normalizes every preflight boolean, fail-closed verifies the exact A0-registry tables, RLS and operation helpers before the 26-file replay, and invokes the exact-match Playwright config without a conflicting positional filter. Bounded Playwright output is retained on failure. It does not mutate S236P, add an eighth historical operation, create another append, or repeat PRE-C3R-P authority or PostgreSQL-oracle work. The already-installed immutable oracle remains a downstream exact-cycle verifier.
