...
This clean replan deterministically establishes the exact A0-registry external Storage tables, RLS, grants and operation helpers with the already-passing Runtime Gate definitions while keeping the unused Storage API excluded, then fail-closed preflights the closed registry before the 26-file replay. PR #799's first two exact-head runs showed that merely enabling the current CLI Storage service did not deterministically establish that database substrate before replay; the final bounded adapter correction removes that startup race and matches the repository's existing isolated-runtime model. It does not mutate S236P, add an eighth historical operation, create another append, or repeat PRE-C3R-P authority or PostgreSQL-oracle work. The already-installed immutable oracle remains a downstream exact-cycle verifier.
