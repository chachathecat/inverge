...
The inherited C3R-P access-identity regression was first aligned from historical `repository.ts` blob `f7f20117c5e3acb14eeb331d8f45a9b97d66c8c2` to the exhaustive item-specific Queue successor and is finally pinned to audited APP-1 replay-safe persistence-authority blob `b808d35f234c9c10163f49bf1f0c420674777853`. The cumulative semantic diff replaces the globally bounded item-detail Queue lookup with a private exact-user/exact-item query, adds the exact pre-insert trusted-Owner, subject and owned-source binding for closed APP-1 metadata, and installs exact-identity ensure operations used by the sealed post-insert replay plan. Every other frozen production-access blob remains unchanged. The regression still hashes every exact file and proves both that `getWrongAnswerDetail` no longer calls `listReviewQueue(userId, 20)` and that APP-1 persistence authority is revalidated before any write client is acquired.
...
The next exact-head review collected three coupled verbal-parser boundaries. Copular arithmetic declarations now accept only the closed currency unit set `원`, `천원`, `만원`, and `억원` between the numeric value and copula, scale both operands and result to exact base units, and therefore recalculate `순수익은 999원이다` without accepting a mismatched unit magnitude. A copular phrase without exactly two local operands and one operative arithmetic verb is ignored rather than treated as arithmetic, so `검토 결과는 2이다` after a valid equation remains ordinary prose and cannot grant or revoke confirmation. Verbal operator selection scans the actual inflected verbs, excludes the closed negated forms, and requires exactly one operative verb; `빼지 않고 더해` therefore evaluates as addition rather than subtraction. Focused hostile coverage binds incorrect and correct unit declarations, unit-magnitude drift, copular prose counts, and negated-operator polarity.

The following exact-head review found three remaining cross-representation cases. Symbolic equations now capture the same closed currency units on both operands and the stated result, require either all three units or none, and compare scaled base-unit values; contextual unit prose cannot make a partially unit-bound equation authoritative. The negation scanner also closes the prohibitive forms `-지 말-`, `-서는 안`, `-선 안`, and `-면 안`, so an earlier correct equation cannot mask a later wrong operative addition. Verbal operand extraction excludes closed year/month/day and count suffixes before enforcing exactly two operands, which keeps `2026년 기준` out of the arithmetic while still recalculating the locally bound result. Correct contextual-year and complete-unit cases confirm; unit drift, prohibitive-negation drift and later contradictory results fail the whole repair.

The next exact-head review completed the remaining numeric-token and dimensional closure. Numeric operands now accept only complete ungrouped decimals or validated three-digit thousands groups and cannot restart after an earlier comma; symbolic expressions cross-check their two locally stated contextual operands before evaluation. Operator-aware currency dimensions permit only like-dimension addition/subtraction, currency-by-scalar multiplication, currency-by-scalar division, or currency-by-currency division to a scalar, rejecting currency-by-currency multiplication even when scaled numbers coincide. The same dimensional guard covers verbal calculations while preserving the established unitless-input/base-`원` result form. The negation scanner recognizes uncontracted `-어서는 안`, and ISO date tokens are removed as whole contextual values before the exactly-two-operand gate. Hostile cases bind the reported grouped-number suffix attack, contextual unit substitution, symbolic and verbal currency multiplication, uncontracted prohibition, and both correct and contradictory ISO-date declarations.

The bounded replacement exact-head review found two final Practice-calculation boundaries. The closed result-noun grammar now includes `감정평가액`, `시산가액`, and `수익가액`; an appraisal-specific copular result without new local operands must cross-bind to the nearest validated symbolic result with compatible currency dimensions, so a correct intermediate equation cannot mask a contradictory final appraisal value. Explicit rounding is accepted only through one closed same-clause declaration for zero through six decimal places (`첫째` through `셋째` or the exact digit form). The validator rounds in the stated result unit before comparison, rejects unexplained discrepancies, malformed or ambiguous rounding declarations, and preserves exact comparison when no rounding is declared. Decimal points no longer split target-specific evidence into false sentence fragments. Focused hostile and positive coverage binds all three appraisal result nouns, unexplained and wrong rounding, unsupported wording, unitless one-decimal division, and currency-unit one-decimal division.

The terminal follow-up correction addresses the remaining deterministic boundaries as one batch. Declared rounding now parses the original decimal tokens into exact BigInt rational arithmetic and applies the repository's `HALF_UP` policy in the declared result unit; it never uses binary-float multiplication plus `Math.round` as authority. The same exact rational comparison is authoritative when no rounding is declared, so `9007199254740992 + 1` accepts only `9007199254740993` and cannot collapse through JavaScript number precision. The focused cases prove `1.005 → 1.01`, `2.675 → 2.68`, and the negative tie `-1.005 → -1.01` at two decimal places, while the corresponding wrong ties, unexplained discrepancies, malformed declarations and dimension drift fail closed and ordinary exact arithmetic remains unchanged. Separately, only an exact `TrustedRepairAccessError` is translated to `APP1_AUTHORITY_REQUIRED`; unrelated errors are rethrown. Initial analysis and repair verification share that source-authority translation, and persistence also translates a revocation raised by the pre-insert repository recheck. Both APP-1 routes return the closed `403` authority code before their generic failure branches, and the existing client transition clears analysis authority, verification, receipt, pending save, conflict and success state while exposing no retryable save action. These deterministic server-boundary changes required no repeated Docker, Supabase or three-viewport browser lifecycle.

The resulting exact-head review found one directly coupled representation boundary: although the symbolic equation itself used exact rational authority, its validated stated result and the later appraisal-result declaration were cross-bound as JavaScript `number` values. The correction preserves the parsed exact decimal through that binding and also removes the same binary-float comparison from contextual-operand-to-symbolic-operand checks. BigInt rational cross-multiplication now rejects both a one-unit contextual substitution and `9007199254740992 + 1 = 9007199254740993` followed by the contradictory `감정평가액은 9007199254740992원이다`; the corresponding exact contextual and appraisal declarations confirm. Unit/dimension compatibility, finite token admission, declared rounding and fail-closed unexplained-discrepancy behavior remain unchanged. The directly affected test, complete APP-1 focused suite `39/39`, typecheck and changed-file lint passed; Docker, Supabase and the complete browser lifecycle were not repeated for this deterministic scanner-only correction.

The subsequent exact-head review found that two common closed appraisal assignment declarations were still outside the result enumeration. The scanner now recognizes `감정평가액을 …원으로 결정` and `감정평가액은 …원으로 산정` forms, including their ordinary inflected continuations, and routes them through the same nearest-validated-symbolic-result, currency-dimension and exact-rational binding used by copular appraisal declarations. Contradictory ordinary and large-integer assignments fail closed; matching `결정` and `산정` declarations confirm. Non-appraisal prose remains outside the appraisal-specific binding branch, and the existing ordinary result-count exclusion is unchanged. The directly affected regression and complete APP-1 focused suite passed `39/39`; typecheck and changed-file lint passed without requiring a repeated runtime lifecycle.

The final polarity follow-up closes the remaining assignment-sequence ambiguity. Appraisal `결정` and `산정` clauses are enumerated separately from ordinary verbal calculations; exact `…하지…` clauses are retained as non-operative declarations, while every later non-negated assignment in the same bounded sentence segment must cross-bind to the nearest validated symbolic result with the existing exact-rational and dimension authority. A correct value inside `100원으로 결정하지 않고 999원으로 결정하였다` can therefore no longer hide the operative contradictory value. Multiple negated declarations followed by a wrong operative assignment also fail closed, while a negated wrong value followed by the exact operative value confirms. Malformed numeric assignment candidates remain fail-closed through exact-versus-candidate enumeration. The directly affected regression and complete APP-1 focused suite passed `39/39`; typecheck and changed-file lint passed, and no browser, provider, database, persistence or runtime boundary changed.

The exact-head polarity review then found that the bare `하지` prefix also occurs inside the affirmative contrastive `하지만`. Assignment negation now requires a complete closed negative continuation: `하지`, optional `는`/`도`, and one of `않`, `못`, `아니`, or `말`. Therefore `999원으로 결정하지만 … 100원으로 산정하였다` validates both operative declarations and fails on the contradictory first value, while `999원으로 결정하지는 않았고 100원으로 산정하였다` continues to ignore only the completed negative declaration and confirms the exact operative value. The directly affected regression, complete APP-1 focused suite, typecheck, changed-file lint and diff check passed; the exact-rational, dimension and server-authority boundaries remain unchanged.

## Final candidate checks

- APP-1 focused suite: `39/39` passed, including `APP1-UI-002B` for revoked-authority exit, `APP1-API-003` for exact access-error translation across initial analysis, verification and persistence, `APP1-VM-003D` for shared persisted-answer selection across substantive `userAnswer`, `rawAnswerText`, flat `rewriteParagraph` and mapped `rawPayload.rewrite_paragraph` fallbacks, `APP1-VM-003` for mapped-rewrite repair anchoring, `APP1-VM-003F` for exhaustive arithmetic and appraisal-result validation without misclassifying ordinary result counts, and `APP1-VM-003G` for exact-decimal closed `HALF_UP` rounding;
- inherited C3R-P/T/L deterministic bundle: `129/129` passed;
