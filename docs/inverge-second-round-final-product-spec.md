...
감정평가사 1차 compatibility 코드는 즉시 삭제하지 않는다. Post-#650에서
공식 규칙·taxonomy·rights/version manifest와 Foundation contract lane만
queued 상태로 허용한다. 1차 learner runtime, 랜딩·온보딩·navigation,
유료 상품, 실제 learner/content 노출은 별도 승인 전까지 허용하지 않는다.
...
product:
  ...
  firstRoundStatus: contracts_lane_queued_runtime_exposure_not_authorized
...
- 모바일 실제 사용자 흐름 통과
- Golden 3와 Golden 9의 별도 O3 content gate 통과
- invitation-only Founding Beta Wave A와 Wave B/C 완료
- second-round authenticated acceptance 통과
- Mineral Cobalt/Figma/home contract readiness 통과
- S225 exact-scope O4 public self-serve 승인

내부 알파와 제한 베타에서는 과목별 순차 검증이 가능하다. 그러나 랜딩, 가격표, 결제 화면에서 세 과목 중 일부만 완성된 상태를 전과목 제품처럼 판매하지 않는다.
정확한 현재 순서와 dependency는
`docs/dabangil-unified-program-contract.md`와
`roadmap/active-program.yml`을 따른다.
...
6. 검증형 기준답안 생성 파이프라인
7. 오답노트·개념 그래프
8. Golden/Founding Beta와 authenticated acceptance
9. 요금제·결제·세 과목 통합 E2E와 S225 public self-serve

개발 순서는 순차지만 공개 제품 범위는 항상 전과목이다.
Academy는 이 공개 출시 순서와 분리된 named-partner lane이며 S225의
dependency가 아니다.
...
이 절은 target/source-contract history일 뿐 현재 Academy readiness나
활성 가격을 뜻하지 않는다. 현재 UI/API/DB/RLS/runtime은 이 reset에서
감사·구현·활성화되지 않았다. 시작에는 named partner, one-tenant packet,
권리·privacy/data-processing 조건, 명시적 instructor approval, 그리고
exact-scope O4C가 모두 필요하다.
...
- private raw 답안은 모델 학습에 직접 사용하지 않는다.
- exact-purpose consent는 필요조건일 뿐 충분조건이 아니다.
- O5 training input은 consented pseudonymous non-reconstructive signal 또는
  promoted Cleared Content Bank material로 제한한다.
- 삭제·내보내기 기능을 제공한다.
...
- S222 Academy Console source-contract history
- S223 Three-subject Corpus & Quality Acceptance
- S224 Three-subject Learner E2E Acceptance
- owner-private Golden 3 readiness and content approval
- owner-private Golden 3
- invitation-only Founding Beta core, Wave A, Golden 9, Wave B/C
- authenticated second-round acceptance
- S225 Public Self-Serve Launch Acceptance

S222–S224의 historical completion은 현재 content, commercial, Academy,
efficacy readiness를 자동으로 증명하지 않는다. Founding Beta와 S225는
별도 acceptance이며, 69,000원/30일/20 `usable_review_unit_v1`은 O4 전에는
owner-approved hypothesis일 뿐이다.
...
- 감정평가사 1차 learner runtime·공개 navigation·pricing·content activation
- 다른 전문자격시험
- 라이브 강의
- 전문가 첨삭 중개
- 사람 채점자 marketplace
