...
- 관련 이슈: #431
- 우선순위: 이 문서와 `AGENTS.md`, `roadmap/active-program.yml`이 충돌하면 **이 문서의 제품 결정과 최신 active program을 우선**한다.
...
Inverge가 판매하는 핵심 가치는 다음과 같다.

> 모든 공식 기출을 풀고, 내 답안을 즉시 연습채점·정밀첨삭하며, 검증형 기준답안과 비교하고, 반복해서 틀리는 핵심개념을 자동으로 추적해 다음 재작성까지 연결한다.
...
product:
  name: Inverge
  category: 감정평가사 2차 전과목 답안 완성 OS
  learnerScope:
    - 감정평가실무
    - 감정평가이론
    - 감정평가 및 보상법규
...
- 기출 풀이부터 첨삭·기준답안·재작성까지 한 번에
- 틀린 핵심개념을 자동으로 쌓아 다음 답안에 반영하는 시스템
- 점수만 보여주지 않고, 다음 답안을 실제로 고치게 하는 시스템
...
- 과거 공식 기출을 연도·회차·과목·문항 단위로 제공한다.
- 사용자의 답안을 사진, PDF, 텍스트로 받는다.
- OCR 결과를 사용자가 확인·수정한 뒤 분석한다.
- 과목별 루브릭으로 **연습점수 범위**와 감점 근거를 보여준다.
- 가장 큰 간극, 누락 논점, 약한 계산 단계, 약한 문단을 구체적으로 지적한다.
- 사용자가 답안을 제출한 뒤 **Inverge 검증형 기준답안** 전체를 제공한다.
...
아래 가격은 **초기 list price 가설**이다. 결제 전환, AI 비용, 유지율로 검증하되 제품 구현은 이 구조를 기준으로 한다.
...
- 월 30회 full answer review credits
- 법규·이론·실무 전과목
- 전체 기출 탐색
- 제출 후 검증형 기준답안 전체
- 예상 점수 범위
...
- 월 80회 full answer review credits
- Core 전체 기능
- 상세 루브릭 point ledger
- 재작성 즉시 재채점
- 실무 계산 trace·검산 상세
...
- 한 번의 review credit은 한 문항과 그 답안의 허용된 파일·페이지 한도 내 분석을 의미한다.
- 페이지·파일 한도는 AI 비용과 OCR 안정성에 따라 명시적으로 제한한다.

## 10.5 Founding paid beta

초기 품질 검증 기간에는 다음 가격을 실험할 수 있다.

- Core founding: 월 59,000원
- Intensive founding: 월 109,000원
- 최초 결제자에게 최대 12개월 가격 유지

정가를 영구적으로 낮추지 않는다. 2차 전과목, 전체 기출, 기준답안, 첨삭, 추적 기능을 모두 갖춘 제품은 단순 AI 채팅보다 높은 가격을 받아야 한다.
...
- Core/Intensive 유료 전환율과 해지 사유 기록
...
- S219 Product Catalog / Usage Ledger
...
| 기능 | Free | Core | Intensive | Academy |
|---|---:|---:|---:|---:|
| 전과목 기출 탐색 | 제한 | 전체 | 전체 | 전체 |
| full answer review | 평생 1회 | 월 30회 | 월 80회 | 계약량 |
| 연습점수 범위 | 1회 | 포함 | 상세 | 초안 |
| 정밀첨삭 | 1회 | 포함 | 심층 | 초안·수정 |
| 검증형 기준답안 | 1회 | 포함 | 포함 | 포함 |
| 재작성·재채점 | 1회 | 포함 | 무제한 범위 내 | 과제 관리 |
| 자동 오답노트 | 1회 체험 | 포함 | 포함 | 학생별 |
| 핵심개념 그래프 | 제한 | 포함 | 심층 | 학생·반 |
| 실무 계산 trace | 1회 | 기본 | 상세 | 상세 |
| 모의세트 | 없음 | 기본 | 포함 | 배포 |
| 전문가 검수 서비스 | 없음 | 없음 | 없음 | 학원 자체 승인 |
...
> Inverge는 감정평가사 2차 법규·이론·실무의 모든 공식 기출을 풀고, 연습채점·정밀첨삭·검증형 기준답안·재작성·자동 오답노트·핵심개념 추적을 하나의 반복 학습 루프로 제공하는 전과목 답안 완성 OS다.
