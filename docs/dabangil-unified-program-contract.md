...
Every dependency or adapter has this forward path:
...
Each transition requires version pinning, license/SBOM evidence, model-asset
rights where relevant, a fallback adapter, an activation owner, comparison
...
active before rollback. The minimum safety path is therefore
...
- `ts-fsrs` and `pyBKT` start in learner-hidden shadow.
- IRT/CAT stays offline until sufficient independent attempts and
  contamination-safe held-out data exist.
...
exact-scope O4 approval. Golden 3 O3 approval does not authorize Golden 9, and
