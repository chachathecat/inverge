...
terminal_donor_prs: [770, 780, 782, 783]
dependency_closure_clean_replan_ordinal: 2
dependency_closure_clean_replan_maximum: 2
dependency_closure_terminal_clean_replan: true
terminal_failure_reduction_sequence: [C3R-A0, C3R-A1]
...
PR #783 correctly exhausted its two source corrections and three exact-head
formal reviews, then closed unmerged after final review `4984950563` reported
actionable P0/P1/P2 `0/0/1`. It remains a read-only source-authority donor
alongside PR #782 and terminal runtime donors PR #770 and PR #780. This Work
is clean replan `2/2`, the terminal full source-authority replan for the
mandatory migration-dependency-closure root blocker. The full-sized WCV-C3
runtime delivery shape remains replaced by the structurally smaller, strictly
serial chain:
...
The chain is the already-selected structural reduction of the full-sized WCV-C3
runtime outcome. This source candidate is not a third full-sized runtime
replan. It preserves every learner, evidence, privacy, rights, migration,
...
| Field | PR #770 | PR #780 | PR #782 | PR #783 |
|---|---|---|---|---|
| State | closed, Draft, unmerged | closed, Draft, unmerged | closed, unmerged | closed, unmerged |
| Head | `41fbc60cbebf5463ead483462e5bd92b797c82c4` | `a28c1983a5264f21ed35ab48a465cd9198a46e5b` | `e3609843850ba1f2ce64c291d9b4daae964d2f65` | `7298b147d5f61e9d2eaa915b6b9c05354124add5` |
| Tree | `e7d035d5735e600bf970054666da0b57f05f1abb` | `e589b0e55f93d70df4ef5c0d335d74e4242f91ce` | `20e0f9235b71c3427ee184bf7e02f22f46633ef6` | `e47605343bfdad7d44737756f56e9f555d089e94` |
| Base | `ffdd3dcc2398dd27b991eee0be34f832da0a65b5` | `ffdd3dcc2398dd27b991eee0be34f832da0a65b5` | `ffdd3dcc2398dd27b991eee0be34f832da0a65b5` | `ffdd3dcc2398dd27b991eee0be34f832da0a65b5` |
| Corrections | `2/2` | `2/2` | `2/2` | `2/2` |
| Formal reviews | `2/3` | `0/3` | `2/3` | `3/3` |
...
PR #783's final exact-head review `PRR_kwDOSMHn8M8AAAABKSBPIw`
(REST ID `4984950563`) preserved that dependency closure but found that
case-insensitive quoted-identifier comparison could equate PostgreSQL
identifiers that are distinct. Review thread `PRRT_kwDOSMHn8M6a4MON` and
comment `3823315579` remain unresolved on the read-only donor. This terminal
clean replan corrects the root class with one deterministic identifier lexer,
exact component identities and no secondary case-insensitive identifier path.
...
10. exact PostgreSQL identifier semantics: only unquoted ASCII identifiers
    fold to lowercase, while quoted identifiers retain decoded case and
    identity component by component;
11. the central Runtime Gate's missing closed adapter for the expanded path
   set; and
12. the absence of a successful two-cycle fresh Supabase reset/replay proof.
...
the canonical UTF-8/LF SQL for all 25 live-main migrations. One deterministic
lexer emits separate quoted and unquoted identifier token classes. Unquoted
ASCII identifiers fold to lowercase; quoted identifiers preserve exact case,
decode doubled quotes, and remain atomic so their payload is never rescanned.
Qualified-name components compare independently. Unsupported Unicode-escape
or non-ASCII unquoted identifier forms fail closed. Line and block comments,
ordinary and escape strings, and scalar dollar-quoted values create no SQL
evidence. Dollar bodies are scanned only when grammar establishes an
executable `sql` or `plpgsql` `DO`, function or procedure body. Exact routine
option grammar accepts identifier and legacy ordinary-string language names;
escape-string names fail unsupported. A declared non-SQL language is ignored
as non-executable by this analyzer, while a function/procedure dollar body
without an exact language fails as ambiguous.
The token stream recognizes mixed
keyword casing, multiple declarations, `IF NOT EXISTS`, `SCHEMA` and `WITH
SCHEMA`, and fails closed on an unknown or ambiguous declaration. SQL-derived
 created extensions, required extension
 uses, exact schemas and exact producers must equal the manifest in both
 directions. Extension use before the corresponding current-file declaration
 fails closed. `DROP EXTENSION` and `ALTER EXTENSION … SET SCHEMA` are outside
 this bounded state model and fail explicitly as unsupported rather than
 preserving a stale producer or schema. Unqualified `digest` is a `pgcrypto` dependency and a qualified
`extensions.digest` call is counted only in its qualified dependency bucket.
Statement-aware type classification recognizes bare, qualified, array,
typmod, both `CREATE CAST` types, expression-`CAST` targets, routine
parameter/return, table/composite column, domain, PL/pgSQL declaration,
PREPARE parameter, routine `TRANSFORM FOR TYPE`, ordinary and ordered-set aggregate input/state, range
subtype, operator argument, operator-class type/storage and both operand and
support-function signatures, operator-family signatures, type-object elements,
foreign-table columns, the exact `TRANSFORM FOR type_name` target grammar and
 `ALTER EXTENSION` TRANSFORM/CAST/AGGREGATE/OPERATOR member signatures for
 `vector`, plus ordinary DROP/ALTER/COMMENT CAST, aggregate and operator
 identity signatures. The `ALTER EXTENSION` operation is consumed only after
 one exact extension-name token, even when that valid unquoted name is `add` or
 `drop`; unary operator identities recognize `NONE` on either operand side.
 `CAST` uses require the one top-level `AS` separator, so nested
 source aliases are not targets; expression-`CAST` source operands, column names, prepared-statement or
 channel names, defaults, numeric function arguments or ordinary aliases do not
 count. Routine `RETURNS` is limited to the post-signature CREATE-routine option
 and typed-table `OF` is limited to the position immediately after the table
 target. It parses
COPY/VACUUM/ANALYZE/CLUSTER/TRUNCATE,
GRANT/REVOKE target lists, `ONLY`, `INHERITS`, MERGE target/source, DELETE
USING, trigger and rule relation positions before generic call/type
classification. CREATE-side `LIKE`, `PARTITION OF`, range-subtype, operator-
implementation and routine-`SUPPORT` dependencies are typed explicitly; `LIKE`
is limited to an exact table-definition element rather than an expression
operator. Producer closure includes temporary and foreign table forms,
temporary and recursive views, domains as types and procedures as functions.
`ALTER` and multi-target `DROP FOREIGN TABLE` use table-kind transitions.
Multi-target drops enumerate every target; object rename or set-schema changes
are an old-identity drop plus a new-identity producer/provenance transition.
Routine identity uses canonical input types across FUNCTION, PROCEDURE and
generic ROUTINE operations, including supported compound names, catalog and
built-in aliases, `oid`, `regclass`, interval field syntax, float precision,
array and typmod equivalence. Only exact closed catalog names may use an
unqualified quoted form. Unqualified custom routine parameter, return and
`RETURNS TABLE` types and every overload or syntax ambiguity fail closed; a
qualified custom type remains a typed provenance dependency. The same analyzer
preserves object kind and source statement order while deriving every reference to the
closed set of prior repository objects and external database objects,
independently derives `consumes`, `modifies`, `drops` and each record's exact
dependency predecessors from SQL and object provenance, and compares those
results in both directions. Exact current-producer self references are valid
 but non-reportable, including a valid relation-context recursive-view self
 reference; a same-named function call remains exact function provenance.
 Aggregate and operator-class/family targets, which are intentionally outside
 the produced-object registry, are likewise suppressed only at their exact
 target positions while their type and function dependencies remain reportable.
Stored-routine body DDL remains dependency evidence rather than migration-time
transition evidence and its references validate against final migration state.
Grammar-established nested executable bodies are discovered recursively. A
nested `DO` inherits stored-routine reference-only ownership from a stored
routine ancestor and conditional/terminating/exception control from an
executed `DO` ancestor. Straight-line DDL inside an executed `DO` body
participates in the same source-ordered producer/modify/drop state as outer
SQL. PL/pgSQL control is classified at each transition position rather than
body-wide. Exception control is limited to its exact
 `BEGIN … EXCEPTION … END` block, including a block or loop with an exact
 PL/pgSQL `<<label>>` prefix, leaving transitions before a later independent
block or after its completion straight-line. Conditional create/drop/rename and any transition after an
unmodelled terminating `RETURN`/`EXIT` fail closed, while a conditional modify
is not published as an unconditional transition. SQL `CASE` expressions and
DDL before or after a completed control block remain straight-line. Known produced functions, including a routine
moved by `SET SCHEMA`, are excluded from the external-function registry. Its
closed dependency classes, parser contract,
extension registry, external-function registry and exact comparison rules
must match the machine manifest exactly. The only two policy-ordering overrides are
...
`codex/wcv-c3r-terminal-identifier-replan` in the same repository, and title
`[WCV-C3R] Install serial structural recovery authority — terminal clean
replan`; an unrelated or
...
If this source-only recovery PR exhausts either budget with a mandatory
actionable P0/P1/P2, it closes unmerged, remains read-only, starts no C3R-P
runtime, and reports the exact conflict. No third full source-authority clean
replan is authorized. From refreshed `main`, structural reduction must instead
deliver `C3R-A0`, an independently complete PostgreSQL migration-dependency
authority and exact identifier lexer/manifest validator, followed only after
its validated merge by `C3R-A1`, an independently complete serial
C3R-P/C3R-T/C3R-L program authority. No stage boundary or mandatory
migration-history gate may be weakened.
...
- PR #770, PR #780, PR #782 and PR #783 remain closed, unmerged and read-only;
