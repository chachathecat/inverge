...
Every nested dollar-quoted body is inspected recursively. Static `INSERT`,
`UPDATE` and `DELETE` are accepted only when their exact schema-qualified
target is a relation created by the same top-level append. An unresolved,
unqualified, historical or otherwise pre-existing target fails closed at any
