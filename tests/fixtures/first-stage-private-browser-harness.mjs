...
export async function verifyPrivateBrowser({ route, clock, failNextWrite, subject = "economics_principles", blockCatalog, blockedMessage, expectedAttributions, questionNumber = 1, retryChoice = 2, ownerLocalTrial = false, bankPractice = false }) {
  ...
  const apiPath = ownerLocalTrial ? "/api/review-os/first-stage/economics-trial/sessions" : subject === "economics_principles" ? "/api/review-os/first-stage/sessions" : `/api/review-os/first-stage/${slug}/sessions`;
  const bundle = await build({ stdin: {
    contents: `import React from "react"; import {createRoot} from "react-dom/client"; import {FirstStagePrivatePractice} from "./components/review-os/first-stage-private-practice"; createRoot(document.getElementById("root")).render(React.createElement(FirstStagePrivatePractice, {subject: ${JSON.stringify(subject)}, ownerLocalTrial: ${ownerLocalTrial}}));`,
  ...
  const server = createServer(async (incoming, outgoing) => {
    try {
      ...
      if (method === "POST" && loseCreateResponse) {
        ...
        // know the result. Retry/reload must preserve that exact create intent.
        assert.equal(response.status,200);
        outgoing.writeHead(503,{"content-type":"application/json","cache-control":"no-store"});
        outgoing.end(JSON.stringify({ok:false,error:"temporarily_unavailable"}));return;
      }
  ...
  try {
    ...
    if (ownerLocalTrial) {
      await page.getByLabel("시험 문항 선택").selectOption(`qnet-2025-36-s1-A-${questionNumber}`);
    ...
    if (ownerLocalTrial) {
      ...
      const popup=await popupPromise;await popup.getByLabel("시험 문항 선택").waitFor();
      assert.equal(page.url(),reconnectUrl);
      assert.equal(await popup.getByLabel("시험 문항 선택").locator("option").count(),4);
      assert.equal(await popup.evaluate(()=>window.opener===null),true);await popup.close();
    }
