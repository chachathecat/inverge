...
test("S232E.3 gives Answer Review one labelled learner-first V3 entry", () => {
  ...
  assert.match(
    source,
    /<h1 id="s232e3-answer-review-title" className="v3-type-screen[^"]*">[\s\S]*?답안 검토[\s\S]*?<\/h1>/,
  );
  assert.equal((source.match(/<h1\b/g) ?? []).length, 1);
  for (const role of ["v3-type-screen", "v3-type-section", "v3-type-body", "v3-type-compact", "v3-type-caption"]) {
    assert.ok(source.includes(role), `missing V3 typography role: ${role}`);
  }
});

test("S232E.3 explains the entry in now, why, result order", () => {
  const context = sliceBetween("data-s232e3-answer-review-context", "<TrustProvenanceLayer");
  ...
  assert.ok(order.every((index) => index >= 0), `missing stage marker: ${order.join(",")}`);
  assert.deepEqual(order, [...order].sort((left, right) => left - right));
  for (const copy of ["지금", "왜", "결과", "내 답안을 스냅하거나 텍스트로 남깁니다.", "내 답안의 누락 논점과 약한 구조를 먼저 좁힙니다.", "가장 큰 간극 1개와 다시 쓸 문장을 확인합니다."]) {
...
test("S232E.3 keeps only answer snap and text in the default entry actions", () => {
  ...
  assert.ok(actionStart >= 0 && actionEnd > actionStart, "missing default answer entry action group");
  ...
  assert.ok(requiredIndex >= 0 && startIndex > requiredIndex && optionalIndex > startIndex);
  assert.match(source, /data-s232e3-answer-required[\s\S]*?data-testid="answer-review-my-answer-input"/);
  assert.equal((source.match(/data-testid="answer-review-start"/g) ?? []).length, 1);
  assert.equal((source.match(/data-s232e3-answer-review-start-surface/g) ?? []).length, 1);
  assert.equal((source.match(/data-s232e3-answer-review-start(?:\s|>)/g) ?? []).length, 1);
});

test("S232E.3 keeps accuracy inputs inside one closed optional disclosure", () => {
  const opening = source.match(/<details[\s\S]*?data-s232e3-answer-review-optional[\s\S]*?>/);
  assert.ok(opening, "missing optional accuracy details");
  assert.doesNotMatch(opening[0], /\sopen(?:=|\s|>)/);

  const optional = sliceBetween("data-s232e3-answer-review-optional", "</motion.div>");
  ...
  for (const preserved of [
    ...
    assert.ok(optional.includes(preserved), `optional input moved or removed: ${preserved}`);
  }
});
...
test("S232E.3 preserves Problem Snap handoff values and notice", () => {
  ...
  for (const mapping of [
    ...
    assert.ok(handoffEffect.includes(mapping), `Problem Snap handoff contract missing: ${mapping}`);
  }
  ...
  assert.ok(
    source.indexOf("{problemSnapNoticeVisible ? (") < source.indexOf("data-s232e3-answer-review-optional"),
...
test("S232E.3 runtime gate is exact-head, same-document, read-only, and scalar-only", () => {
  ...
  assert.doesNotMatch(spec, /requiredAnswer\.(?:fill|type|pressSequentially)\(/);
  ...
  assert.match(workflow, /path: validated-runtime-evidence\/s232e3-runtime\.json/);
