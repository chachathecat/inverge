...
test("Owner home keeps all five subject routes bodyless and one primary fallback order", () => {
  ...
  assert.match(componentSource, /예정 복습 시작/u);
...
test("real browser selects reviewed stock, then local trial, then second-stage handoff", async () => {
  ...
  try {
    ...
    scenario = "bank-due";
    await verify("새 연습 가능 0/5과목", "경제학 예정 복습 시작", "/app/first-stage/practice?sessionId=bank-saved");
    ...
    scenario = "priority";
    await verify("새 연습 가능 2/5과목", "회계학 예정 복습 시작", "/app/first-stage/accounting?sessionId=accounting-high-review");
    scenario = "ready-versus-due";
    await verify("새 연습 가능 2/5과목", "경제학 예정 복습 시작", "/app/first-stage/practice?sessionId=due-session");
    scenario = "tie";
    await verify("새 연습 가능 2/5과목", "경제학 예정 복습 시작", "/app/first-stage/practice?sessionId=z-session");
    scenario = "future";
    ...
    scenario = "due";
    await verify("새 연습 가능 1/5과목", "경제학 예정 복습 시작", "/app/first-stage/practice?sessionId=economics-session-1");
    scenario = "reviewed";
