...
import { expectedCommitmentForSubject } from "../lib/review-os/durable-learning-fixtures.ts";
...
function successfulStage(aggregate, prepareAt, submitAt) {
  ...
  return apply(prepared, planDurableEvidence({
    aggregate: prepared,
    commitment: expectedCommitmentForSubject(prepared.caseRecord.subject),
...
test("WCV-C3 rejects an instant submission and preserves explicit typed failure", () => {
  ...
  assert.throws(() => planDurableEvidence({
    aggregate: prepared,
    commitment: expectedCommitmentForSubject("appraisal_practical"),
    body: "too fast",
    occurredAt: "2026-08-18T00:00:01.000Z",
  }), /not_eligible/);
  const wrong = { ...expectedCommitmentForSubject("appraisal_practical"), result: 99 };
...
test("WCV-C3 rejects a same-surface or pre-exposed transfer assignment and preserves timeout", () => {
  ...
  assert.throws(() => planDurableEvidence({ aggregate: sameSurface, commitment: expectedCommitmentForSubject("appraisal_practical"), body: "same surface", occurredAt: "2026-08-24T00:02:01.000Z" }), /stale_configuration/);
  const exposed = {
    ...prepared,
    caseRecord: {
      ...prepared.caseRecord,
  ...
  assert.throws(() => planDurableEvidence({ aggregate: exposed, commitment: expectedCommitmentForSubject("appraisal_practical"), body: "pre-exposed", occurredAt: "2026-08-24T00:02:01.000Z" }), /stale_configuration/);

  aggregate = successfulStage(aggregate, "2026-08-24T00:00:01.000Z", "2026-08-24T00:02:01.000Z");
  const timed = apply(aggregate, planAttemptPreparation({ aggregate, occurredAt: "2026-08-24T00:03:00.000Z" }));
  const timeout = apply(timed, planDurableEvidence({ aggregate: timed, commitment: expectedCommitmentForSubject("appraisal_practical"), body: "late timed response", occurredAt: "2026-08-24T00:33:01.000Z" }));
...
test("WCV-C3 later qualifying independent failure reopens currently-clear evidence", () => {
  ...
  const wrong = { ...expectedCommitmentForSubject("appraisal_theory"), polarity: "NEGATIVE" };
