...
function structuredClaim(view: {
  ...
  return {
    ...
    result: { value: resultValue, unit: confirmation.unit },
...
async function completePracticeJourney(input: {
  ...
  await expect(page.getByLabel("계산관계 검증 상태")).toHaveCount(0);
  ...
  await fillText(page, "총수익 원/년", "120000000", input.keyboardOnly);
  await fillText(page, "운영비 원/년", "20000000", input.keyboardOnly);
  await fillText(page, "결과 원/년", "100000000", input.keyboardOnly);
  await activatePrimary(page, input.keyboardOnly);
...
async function createPartialPractice(context: BrowserContext) {
  ...
  const confirmation = await apiCommand(context, "confirm_claim", {
    sessionId: view.session.sessionId,
    expectedVersion: view.session.recordVersion,
    claim: structuredClaim(view, 90000000),
  });
  expect(confirmation.response.status()).toBe(200);
  view = confirmation.body.view;
  expect(view.session.state).toBe("partial");
  expect(view.session.proofEvaluation.state).toBe("PARTIAL");
...
test("Practice-only browser to Postgres journey, hostile concurrency, bounded retry, and tenant isolation", async ({ browser }) => {
  ...
  await retryPage.unroute("**/api/review-os/trusted-repair");

  const retryableServerCommandIds: string[] = [];
  let replaceFirstCommittedResponseWith503 = true;
  await retryPage.route("**/api/review-os/trusted-repair", async (route) => {
    const payload = route.request().postDataJSON() as {
      action?: string;
      commandId?: string;
    };
    if (payload.action !== "commit_prediction") {
      await route.continue();
      return;
    }
    retryableServerCommandIds.push(payload.commandId ?? "");
    if (replaceFirstCommittedResponseWith503) {
      replaceFirstCommittedResponseWith503 = false;
      const committed = await route.fetch();
      expect(committed.status()).toBe(200);
      await route.fulfill({
        status: 503,
  ...
  await expect(retryPage.getByRole("alert")).toBeVisible();
  await activatePrimary(retryPage);
  await expectState(retryPage, "prediction_committed");
  expect(retryableServerCommandIds).toHaveLength(2);
  expect(retryableServerCommandIds[0]).not.toBe("");
  ...
  const identityRetryOwner = await contextFor(browser, emailA, passwordA);
  const identityRetryPage = await identityRetryOwner.newPage();
  const identityRetryCommandIds: string[] = [];
  let replaceOwnerAStartWith503 = true;
  await identityRetryPage.route(
    "**/api/review-os/trusted-repair",
    async (route) => {
      const payload = route.request().postDataJSON() as {
        action?: string;
      ...
      identityRetryCommandIds.push(payload.commandId ?? "");
      if (replaceOwnerAStartWith503) {
        replaceOwnerAStartWith503 = false;
        const committed = await route.fetch();
        expect(committed.status()).toBe(200);
        await route.fulfill({
          status: 503,
  ...
  await identityRetryPage.goto("/app/trusted-repair");
  await activatePrimary(identityRetryPage);
  await expect(identityRetryPage.getByRole("alert")).toBeVisible();
  const ownerABinding = await identityRetryPage.evaluate(() => {
    const encoded = window.sessionStorage.getItem(
      "inverge:c2r-c-p:pending-command:v1",
    );
    return encoded ? (JSON.parse(encoded) as Record<string, unknown>) : null;
  });
  expect(ownerABinding).not.toBeNull();
  expect(Object.keys(ownerABinding ?? {}).sort()).toEqual([
    "commandId",
    "fingerprint",
    "ownerScope",
  ]);
  await login(identityRetryOwner, emailB, passwordB);
  await identityRetryPage.reload();
  await activatePrimary(identityRetryPage);
  await expectState(identityRetryPage, "editable_capture_draft");
  expect(identityRetryCommandIds).toHaveLength(2);
  expect(identityRetryCommandIds[0]).not.toBe("");
  expect(identityRetryCommandIds[1]).not.toBe(identityRetryCommandIds[0]);
  await expect
    .poll(() =>
      identityRetryPage.evaluate(() =>
    ...
    .toBeNull();
  await identityRetryOwner.close();
  ...
  const failedVerification = await apiCommand(owner, "confirm_claim", {
    sessionId: failedRetry.body.view.session.sessionId,
    expectedVersion: failedRetry.body.view.session.recordVersion,
    claim: structuredClaim(failedRetry.body.view, 90000000),
  });
  ...
  const evidence = {
    ...
    exactStructuredPracticeClaimPass: "passed",
    disconnectedNumbersFailClosed: "passed",
    boundedPartialRetry: "passed",
    casAndIdempotency: "passed",
    indeterminateRetryIdReuse: "passed",
    ownerBoundIndeterminateRetry: "passed",
