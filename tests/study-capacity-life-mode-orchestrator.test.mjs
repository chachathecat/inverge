...
test("day plans reject required scope beyond the accepted seven-day Plan Gap bound", () => {
  const boundedAvailability = availability({
    declaredActiveMinutes: 720,
    windows: [{ id: "plan-gap-bound", startMinute: 0, endMinute: 720, environment: "desk", interruptibility: "low" }],
  });
  const boundedCandidates = Array.from({ length: 7 }, (_, index) => task(`bounded-required-${index}`, {
    estimatedMinutes: 720,
    requiredness: "required",
    prioritySignals: ["due_review"],
  }));
  const boundedPlan = buildStudyDayPlan({ profile: profile(), availability: boundedAvailability, candidates: boundedCandidates });

  assert.equal(boundedPlan.planGap?.requiredPlanMinutes, 5040);
  assert.doesNotThrow(() => replanStudyDay({
    previousPlan: boundedPlan,
    newAvailability: boundedAvailability,
  ...
  assert.throws(() => buildStudyDayPlan({
    ...
    candidates: [...boundedCandidates, task("required-over-seven-day-bound", {
      estimatedMinutes: 1,
      requiredness: "required",
      prioritySignals: ["due_review"],
    })],
  }), /required-plan-minutes-exceed-seven-day-bound:5041/);
});

test("week plans reject dates that do not fit in one seven-day interval", () => {
  const weekDay = (date) => availability({
    date,
    declaredActiveMinutes: 60,
    windows: [{ id: `week-${date}`, startMinute: 600, endMinute: 660, environment: "desk", interruptibility: "low" }],
  });

  assert.doesNotThrow(() => buildStudyWeekPlan({
    profile: profile(),
    days: [weekDay("2026-08-25"), weekDay("2026-08-31")],
    candidates: [],
    requiredMinimumMinutes: 0,
    requiredMaximumMinutes: 120,
  }));
  assert.throws(() => buildStudyWeekPlan({
    profile: profile(),
    days: [weekDay("2026-08-25"), weekDay("2026-09-01")],
    candidates: [],
    requiredMinimumMinutes: 0,
    requiredMaximumMinutes: 120,
  }), /week-dates-outside-seven-day-interval/);
});
