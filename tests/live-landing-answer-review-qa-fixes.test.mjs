...
test("answer review quick actions stay prioritized and mobile clear", () => {
  ["답안 스냅으로 시작", "사례 스캔", "PDF/사진 불러오기", "텍스트 입력", "scrollIntoView", "focus"].forEach((token) => {
...
test("second write entry cards remain separated by intent", () => {
  ["답안 스냅 검토", "사례 스캔", "새 답안 작성", "문제 먼저 이해하기", "2차 답안 작성 워크스페이스"].forEach((token) => {
...
test("guardrails and scope boundaries remain enforced", () => {
  const blocked = [
    "확정 점수", "모범답안 확정", "official grader", "pass/fail judge", "정답 보장", "합격 보장",
  ...
  assert.doesNotMatch(combined, /공식 채점(?!\s*아님|이나)/);
  assert.doesNotMatch(combined, /합격 판정(?!이 아닙니다|이 아님| 아님|이 아니라)/);
});
