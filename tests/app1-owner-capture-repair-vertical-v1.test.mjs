...
test("APP1-VM-003F recalculates closed symbolic and verbal Practice arithmetic", () => {
  ...
  for (const repairText of [
    ...
    "수익 1,200,000원과 비용 200,000원의 계산 산식은 1,200,000원 - 200,000원 = 0원이고 단위와 부호를 검산했다.",
    "금액 2만원과 금액 3만원의 계산 산식은 2만원 * 3만원 = 6억원이고 단위와 부호를 검산했다.",
    "수익 2만원과 비용 3만원을 곱해 순수익은 6억원이다, 단위와 부호를 검산했다.",
  ...
  const correctSymbolicBoundedUnits =
    "수익 120만원과 비용 20만원의 계산 산식은 120만원 - 20만원 = 100만원이고 단위와 부호를 검산했다.";
  assert.equal(
    evaluateApp1SameSessionRepair({
      detail,
      requestedGap,
      repairText: correctSymbolicBoundedUnits,
      repairDraft: resolvedTargetDraft({
        strength: correctSymbolicBoundedUnits,
      }),
    }).state,
    "repair_confirmed_for_this_session",
  );
  const correctGroupedSymbolicUnits =
    "수익 1,200,000원과 비용 200,000원의 계산 산식은 1,200,000원 - 200,000원 = 1,000,000원이고 단위와 부호를 검산했다.";
  assert.equal(
    evaluateApp1SameSessionRepair({
      detail,
      requestedGap,
      repairText: correctGroupedSymbolicUnits,
      repairDraft: resolvedTargetDraft({
        strength: correctGroupedSymbolicUnits,
      }),
    }).state,
    "repair_confirmed_for_this_session",
  );
  const correctDimensionedMultiplication =
    "수익 3만원과 비용 배수 2의 계산 산식은 3만원 * 2 = 6만원이고 결과 단위와 부호를 검산했다.";
  assert.equal(
    evaluateApp1SameSessionRepair({
      detail,
      requestedGap,
      repairText: correctDimensionedMultiplication,
      repairDraft: resolvedTargetDraft({
        strength: correctDimensionedMultiplication,
      }),
    }).state,
    "repair_confirmed_for_this_session",
  );
  const correctUncontractedNegation =
    "수익 120에서 비용 20을 빼어서는 안 되고 더해 순수익은 140이다, 단위와 부호를 검산했다.";
  assert.equal(
    evaluateApp1SameSessionRepair({
      detail,
      requestedGap,
      repairText: correctUncontractedNegation,
      repairDraft: resolvedTargetDraft({
        strength: correctUncontractedNegation,
      }),
    }).state,
    "repair_confirmed_for_this_session",
  );
  ...
  const correctContextualIsoDate =
    "수익 120에서 비용 20을 빼고 2026-08-26 기준 순수익은 100이다, 단위와 부호를 검산했다.";
  assert.equal(
    evaluateApp1SameSessionRepair({
      detail,
      requestedGap,
      repairText: correctContextualIsoDate,
      repairDraft: resolvedTargetDraft({
...
test("APP1-VM-003G honors only closed explicit Practice rounding", () => {
  ...
  for (const repairText of [
    ...
    "수익 -1.005와 환원 배수 0의 계산 산식은 -1.005 + 0 = -1.00이고 소수점 둘째 자리까지 반올림했으며 단위와 부호를 검산했다.",
    "수익 9007199254740992와 환원 배수 1의 계산 산식은 9007199254740992 + 1 = 9007199254740992이고 단위와 부호를 검산했다.",
    "수익 9007199254740992와 환원 배수 1의 계산 산식은 9007199254740993 + 1 = 9007199254740994이고 단위와 부호를 검산했다.",
    "수익 9007199254740992와 환원 배수 1의 계산 산식은 9007199254740992 + 1 = 9007199254740993이고 감정평가액은 9007199254740992원이다, 단위와 부호를 검산했다.",
    "수익 100과 환원 배수 1의 계산 산식은 100 / 1 = 100이고 감정평가액을 999원으로 결정하였다, 단위와 부호를 검산했다.",
    "수익 100과 환원 배수 1의 계산 산식은 100 / 1 = 100이고 감정평가액은 999원으로 산정하였다, 단위와 부호를 검산했다.",
    "수익 100과 환원 배수 1의 계산 산식은 100 / 1 = 100이고 감정평가액을 1,00원으로 결정하였다, 단위와 부호를 검산했다.",
    "수익 100과 환원 배수 1의 계산 산식은 100 / 1 = 100이고 감정평가액을 100원으로 결정하고 999원으로 산정하였다, 단위와 부호를 검산했다.",
    "수익 100과 환원 배수 1의 계산 산식은 100 / 1 = 100이고 감정평가액을 999원으로 결정하지만 최종적으로 100원으로 산정하였다, 단위와 부호를 검산했다.",
    "수익 100과 환원 배수 1의 계산 산식은 100 / 1 = 100이고 감정평가액을 100원으로 결정하지 않고 999원으로 결정하였다, 단위와 부호를 검산했다.",
    "수익 100과 환원 배수 1의 계산 산식은 100 / 1 = 100이고 감정평가액을 100원으로 결정하지 않고 200원으로 산정하지 않고 999원으로 결정하였다, 단위와 부호를 검산했다.",
    "수익 9007199254740992와 환원 배수 1의 계산 산식은 9007199254740992 + 1 = 9007199254740993이고 감정평가액을 9007199254740992원으로 결정하였다, 단위와 부호를 검산했다.",
  ]) {
    assert.equal(
      evaluateApp1SameSessionRepair({
        detail,
  ...
  for (const repairText of [
    ...
    "수익 9007199254740992와 환원 배수 1의 계산 산식은 9007199254740992 + 1 = 9007199254740993이고 단위와 부호를 검산했다.",
    "수익 9007199254740993와 환원 배수 1의 계산 산식은 9007199254740993 + 1 = 9007199254740994이고 단위와 부호를 검산했다.",
    "수익 9007199254740992와 환원 배수 1의 계산 산식은 9007199254740992 + 1 = 9007199254740993이고 감정평가액은 9007199254740993원이다, 단위와 부호를 검산했다.",
    "수익 100과 환원 배수 1의 계산 산식은 100 / 1 = 100이고 감정평가액을 100원으로 결정하였다, 단위와 부호를 검산했다.",
    "수익 100과 환원 배수 1의 계산 산식은 100 / 1 = 100이고 감정평가액은 100원으로 산정하였다, 단위와 부호를 검산했다.",
    "수익 100과 환원 배수 1의 계산 산식은 100 / 1 = 100이고 감정평가액을 100원으로 결정하지만 최종적으로 100원으로 산정하였다, 단위와 부호를 검산했다.",
    "수익 100과 환원 배수 1의 계산 산식은 100 / 1 = 100이고 감정평가액을 999원으로 결정하지 않고 100원으로 결정하였다, 단위와 부호를 검산했다.",
    "수익 100과 환원 배수 1의 계산 산식은 100 / 1 = 100이고 감정평가액을 999원으로 결정하지는 않았고 100원으로 산정하였다, 단위와 부호를 검산했다.",
    "수익 9007199254740992와 환원 배수 1의 계산 산식은 9007199254740992 + 1 = 9007199254740993이고 감정평가액을 9007199254740993원으로 결정하였다, 단위와 부호를 검산했다.",
  ]) {
    assert.equal(
      evaluateApp1SameSessionRepair({
        detail,
