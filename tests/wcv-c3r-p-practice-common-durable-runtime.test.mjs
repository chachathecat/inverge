...
test("C3R-P applies the exact seven operations and one 26th append", () => {
  ...
  assert.equal(contract.migrationAuthorityBinding.operationBindings.length, 7);
  assert.equal(contract.migrationAuthorityBinding.appendPath, C3R_P_APPEND_PATH);
  assert.ok(inventory.some((entry) => entry.path === C3R_P_APPEND_PATH && entry.sha256.length === 64));
  for (const operation of contract.migrationAuthorityBinding.operationBindings) {
    const bytes = fs.readFileSync(path.join(root, operation.futurePath));
    ...
    if (operation.currentPath !== operation.futurePath) {
      assert.equal(fs.existsSync(path.join(root, operation.currentPath)), false);
    }
  }
});
