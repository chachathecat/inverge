...
import {
  ...
  ORACLE_PLATFORM,
  ORACLE_PG15_8_REVOKE_DELTA_SHA256,
  ORACLE_PG15_8_REVOKE_FIXTURE_ID,
  ORACLE_PG15_8_REVOKE_MEMBERSHIP_DELTA_SHA256,
  ORACLE_POSTGRES_SOURCE_COMMIT,
  ORACLE_POSTGRES_SOURCE_TAG,
...
function validEvidence(riskBytes) {
  ...
  const fixtureReceipts = oracleFixtureSteps().map((step, index) => {
    ...
    const isPg15_8Revoke = step.id === ORACLE_PG15_8_REVOKE_FIXTURE_ID;
    const after = step.expectation === "rejection"
      ? before
      : isPg15_8Revoke
        ? {
            ...before,
            memberships: sha256(Buffer.from(`snapshot-${index + 1}:memberships`, "utf8")),
          }
        : digestMap(`snapshot-${index + 1}`);
    current = after;
    const delta = isPg15_8Revoke
      ? {
          collectionCounts: Object.fromEntries(ORACLE_SNAPSHOT_COLLECTIONS.map((collection) => [
            collection,
            collection === "memberships"
              ? { added: 0, changed: 0, removed: 1 }
              : { added: 0, changed: 0, removed: 0 },
          ])),
          collectionDigests: Object.fromEntries(ORACLE_SNAPSHOT_COLLECTIONS.map((collection) => [
            collection,
            collection === "memberships"
              ? ORACLE_PG15_8_REVOKE_MEMBERSHIP_DELTA_SHA256
              : sha256(Buffer.from(canonicalJson({ added: [], changed: [], removed: [] }), "utf8")),
          ])),
          sha256: ORACLE_PG15_8_REVOKE_DELTA_SHA256,
        }
      : syntheticDeltaMetadata(before, after);
    return {
...
test("oracle risk classification is exact, complete, and closed to eleven paths", () => {
  const independentlyFrozenPaths = [
  ...
  assert.equal(independentlyFrozenPaths.length, 11);
  assert.deepEqual(ORACLE_ALLOWED_CHANGED_PATHS, independentlyFrozenPaths);
...
test("PostgreSQL 15.8 binds the exact GRANT/REVOKE nonexistent-GRANTED-BY asymmetry", () => {
  const steps = oracleFixtureSteps();
  const grant = steps[20];
  const revoke = steps[21];
  assert.equal(steps.length, 61);
  assert.deepEqual({
    expectation: grant.expectation,
    id: grant.id,
    inputSha256: grant.inputSha256,
    ordinal: grant.ordinal,
    sqlstates: grant.sqlstates,
  }, {
    expectation: "rejection",
    id: "membership_granted_by_nonexistent_rejection",
    inputSha256: "233151b89bf712dbd0067d783d142ab24350d59b38e2736c4e155c8f3f06d251",
    ordinal: 21,
    sqlstates: ["42704"],
  });
  assert.deepEqual({
    expectation: revoke.expectation,
    expectedStdoutSha256: revoke.expectedStdoutSha256,
    id: revoke.id,
    inputSha256: revoke.inputSha256,
    ordinal: revoke.ordinal,
    sqlstates: revoke.sqlstates,
    stdout: revoke.stdout,
  }, {
    expectation: "success",
    expectedStdoutSha256: "9a271f2a916b0b6ee6cecb2426f0b3206ef074578be55d9bc94f6f3fe3ab86aa",
    id: ORACLE_PG15_8_REVOKE_FIXTURE_ID,
    inputSha256: "3f769f605b508697eb57536ae5549f91c7da9519048cd5219b0e55dfb40bee37",
    ordinal: 22,
    sqlstates: [],
    stdout: ["0"],
  });
  assert.match(revoke.sql, /^DO \$oracle\$[\s\S]*required membership precondition is absent[\s\S]*\$oracle\$;\nREVOKE oracle_grantor FROM oracle_direct GRANTED BY oracle_missing;\nSELECT count\(\*\) FROM pg_catalog\.pg_auth_members WHERE roleid = 'oracle_grantor'::regrole AND member = 'oracle_direct'::regrole;\n$/u);
  assert.doesNotMatch(revoke.sql, /^GRANT oracle_grantor TO oracle_direct;/u);
  assert.equal(
    oracleManifestContract().fixtureSetSha256,
  ...
  const grantReceipt = evidence.fixtureReceipts[20];
  const revokeReceipt = evidence.fixtureReceipts[21];
  assert.equal(grantReceipt.psqlStatus, 3);
  assert.equal(grantReceipt.sqlstate, "42704");
  assert.equal(grantReceipt.preSnapshotSha256, grantReceipt.postSnapshotSha256);
  assert.equal(revokeReceipt.psqlStatus, 0);
  assert.equal(revokeReceipt.sqlstate, null);
  assert.equal(revokeReceipt.stdoutSha256, revoke.expectedStdoutSha256);
  assert.deepEqual(revokeReceipt.deltaCollectionCounts.memberships, {
    added: 0,
    changed: 0,
    removed: 1,
  });
  assert.equal(
    revokeReceipt.deltaCollectionDigests.memberships,
    ORACLE_PG15_8_REVOKE_MEMBERSHIP_DELTA_SHA256,
  );
  assert.equal(revokeReceipt.deltaSha256, ORACLE_PG15_8_REVOKE_DELTA_SHA256);
  assert.equal(validate(evidence, risk, riskBytes), true);
});

test("manifest and QA pin the exact non-generalizable PostgreSQL 15.8 source behavior", () => {
  const manifest = oracleManifestContract();
  assert.deepEqual(manifest.sourceIdentity.postgresqlSourceProfile, {
    behaviorScope: "role_membership_granted_by_asymmetry_exact_pg15_8_only",
    generalizableToOtherVersions: false,
    imageDigest: ORACLE_IMAGE_DIGEST,
    serverVersionNum: ORACLE_SERVER_VERSION_NUM,
    sourceCommit: ORACLE_POSTGRES_SOURCE_COMMIT,
    tag: ORACLE_POSTGRES_SOURCE_TAG,
  });
  const qa = fs.readFileSync(ORACLE_QA_PATH, "utf8");
  for (const exact of [
    "REL_15_8",
    ORACLE_POSTGRES_SOURCE_COMMIT,
    "server_version_num = 150008",
    ORACLE_IMAGE_DIGEST,
    "GRANT assigns `opt_granted_by` to `stmt->grantor`",
    "REVOKE parses `opt_granted_by` but does not assign it to `stmt->grantor`",
    "resolves the non-null named GRANT grantor",
    "reaches membership deletion without resolving the parsed nonexistent grantor",
    "Only the REVOKE `GRANTED BY` clause is ignored",
    "This is not an authorization bypass",
    "must not be generalized",
  ]) assert.equal(qa.includes(exact), true, exact);
});
...
test("ordinal 22 rejects self-consistent status, stdout, and semantic-delta mutants", () => {
  const risk = requiredRisk();
  const riskBytes = Buffer.from(JSON.stringify(risk), "utf8");
  const mutations = [
    (receipt) => { receipt.psqlStatus = 3; },
    (receipt) => { receipt.sqlstate = "42704"; },
    (receipt) => { receipt.stdoutSha256 = "a".repeat(64); },
    (receipt) => {
      receipt.deltaCollectionCounts.memberships = { added: 0, changed: 1, removed: 0 };
      receipt.deltaSha256 = sha256(Buffer.from(canonicalJson({
        collectionCounts: receipt.deltaCollectionCounts,
        collectionDigests: receipt.deltaCollectionDigests,
      }), "utf8"));
    },
    (receipt) => {
      receipt.deltaCollectionDigests.memberships = "b".repeat(64);
      receipt.deltaSha256 = sha256(Buffer.from(canonicalJson({
        collectionCounts: receipt.deltaCollectionCounts,
        collectionDigests: receipt.deltaCollectionDigests,
      }), "utf8"));
    ...
    (receipt) => {
      receipt.deltaCollectionCounts.roles = { added: 0, changed: 1, removed: 0 };
      receipt.deltaCollectionDigests.roles = "c".repeat(64);
      receipt.postCollectionDigests.roles = "d".repeat(64);
      receipt.postSnapshotSha256 = rootDigest(receipt.postCollectionDigests);
      receipt.deltaSha256 = sha256(Buffer.from(canonicalJson({
        collectionCounts: receipt.deltaCollectionCounts,
        collectionDigests: receipt.deltaCollectionDigests,
      }), "utf8"));
  ...
  for (const mutate of mutations) {
    const mutant = validEvidence(riskBytes);
    mutate(mutant.fixtureReceipts[21]);
    resealEvidence(mutant);
    assert.throws(
      () => validate(mutant, risk, riskBytes),
      /PostgreSQL 15\.8 REVOKE|successful fixture receipt/u,
    );
  }
});
...
function prEvent(body = prBody(), draft = true) {
  return {
    ...
    pull_request: {
      ...
      draft,
      head: {
        ref: "codex/wcv-c3-pre-p-postgresql-oracle-clean-replacement",
        repo: { full_name: "chachathecat/inverge" },
      },
      title: "[WCV-C3 PRE-P] Install PostgreSQL 15.8 security-state oracle tooling — clean replacement",
    },
  };
}
