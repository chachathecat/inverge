...
const DECISION = "docs/decisions/2026-08-17-owner-c2r-c-p-structural-practice-proof-recovery.md";
const CONTRACT = "config/dabangil-c2r-c-p-structural-practice-proof-v2.json";
...
const CANDIDATES = new Map([
  ...
  [2, ["C2R-C-P-R02", "tests/c2r-c-p-practice-structured-proof.test.mjs", []]],
  [4, ["C2R-C-P-R04", "tests/c2r-c-p-practice-structured-proof.test.mjs", ["C2R-C-T", "C2R-C-L"]]],
  [6, ["C2R-C-P-R06", "tests/c2r-c-p-practice-structured-proof.test.mjs", ["C2R-C-T", "C2R-C-L"]]],
  [8, ["C2R-C-P-R08/R11/R14", "tests/c2r-c-p-practice-runtime-contract.test.mjs", ["C2R-C-T", "C2R-C-L"]]],
  [9, ["C2R-C-P-R09", "tests/c2r-c-p-practice-structured-proof.test.mjs", []]],
  [10, ["C2R-C-P-R10", "tests/c2r-c-p-practice-structured-proof.test.mjs", []]],
  [11, ["C2R-C-P-R11", "tests/c2r-c-p-practice-runtime-contract.test.mjs", ["C2R-C-T", "C2R-C-L"]]],
  [12, ["C2R-C-P-R12", "tests/c2r-c-p-practice-structured-proof.test.mjs", []]],
  [14, ["C2R-C-P-R08/R11/R14", "tests/c2r-c-p-practice-runtime-contract.test.mjs", ["C2R-C-T", "C2R-C-L"]]],
  [19, ["C2R-C-P-R19", "tests/c2r-c-p-practice-structured-proof.test.mjs", []]],
]);
...
test("C2R-C-P stays complete through terminal Law and the WCV-C3 completion candidate", async () => {
...
test("C2R-C-P preserves exactly its 11 PR 756 regression declarations", async () => {
  ...
  for (const [number, cells] of rows) {
    ...
    assert.equal(declaration.coveringPrNumber, 756);
...
test("C2R-C-P preserves terminal issue and Production gates", async () => {
  ...
  assert.match(decision, /free-form text parsing cannot safely establish canonical Practice proof/i);
  assert.match(decision, /#703\/#704\/#705\/#706\/#714\/#717 remain\s+open/i);
  assert.equal(stage.practiceProof.canonicalVerificationAuthority, "server_validated_PracticeCalculationClaimV2_only");
  assert.equal(stage.practiceProof.freeFormTextCanCreateVerified, false);
  assert.equal(stage.practiceProof.denylistVocabularyIsVerificationGate, false);
  assert.equal(stage.practiceProof.deterministicValidatorId, "validator:practice-calculation-claim@2");
  assert.equal(stage.ownedPathManifest.length, 49);
  assert.equal(new Set(stage.ownedPathManifest).size, 49);
  assert.ok(stage.ownedPathManifest.includes("tests/c2r-c-p-practice-structured-proof.test.mjs"));
  assert.ok(stage.ownedPathManifest.includes("supabase/migrations/20260817090000_c2r_c_p_structured_practice_proof.sql"));
  assert.equal(stage.ownedPathManifest.some((path) => path.includes("practice-trusted-repair.test")), false);
});
...
test("C2R-C-P machine contract preserves the authoritative learner episode order", async () => {
  ...
  assert.deepEqual(stage.episode.phases, [
    ...
    "STRUCTURED_CALCULATION_CONFIRMATION",
    "DETERMINISTIC_SERVER_VERIFICATION",
    "CONTINUATION_DECISION",
  ]);
  assert.equal(stage.episode.freeFormReconstructionCreatesVerified, false);
  assert.equal(stage.episode.prefillAutomaticallyConfirmed, false);
});
