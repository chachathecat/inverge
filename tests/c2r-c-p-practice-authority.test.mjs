...
test("C2R-C-P authority selects only C2R-C-T after protected Practice completion", async () => {
  ...
  assert.equal(stages.get("C2R-C-P").coveringPr, 745);
...
test("C2R-C-P declares exactly 11 non-effective PR 745 regression candidates", async () => {
  ...
  for (const [number, cells] of rows) {
    ...
    assert.equal(declaration.coveringPrNumber, 745);
  ...
  assert.equal(coverage.activeCandidateCoveringPr, 745);
