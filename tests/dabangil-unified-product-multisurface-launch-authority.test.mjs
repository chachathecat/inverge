...
test("records WCV-C3 completion and no dependency-ready successor after terminal Law", async () => {
  const [
    launch,
    unified,
    roadmapSource,
  ...
  const currentStageMirrors = {
    ...
    launchConvergenceAmendment: {
      ...
      trackerIssue: unified.launchConvergenceAmendment.soleNextImplementationTrackerIssue,
  ...
  assert.equal(authorityGraph.roadmapItemId, "WCV-C2");
  ...
  assert.match(unifiedMarkdown, /No successor is\s+dependency-ready because ULC-M1 additionally requires completed S241A/);
