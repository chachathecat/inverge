...
async function developmentReservations(root, scope = "development") {
  const rows = [];
  for (const name of (await readdir(root)).filter(name => /^call-(?:0[1-9]|1[0-4])\.json$/.test(name))) {
    try { const row = JSON.parse(await readFile(path.join(root, name), "utf8")); if (row.scope === scope) rows.push(row); }
    catch { fail("OWNER_THEORY_UNKNOWN_RESERVATION"); }
  }
  return rows;
}
export async function reserveTheoryDevelopmentCall(root, settings, authority) {
  return reserveDevelopmentCall(root, settings, authority, "development");
}
async function reserveDevelopmentCall(root, settings, authority, scope) {
  validateTheorySettings(settings);
  const practice = scope === "practice_development";
  if (authority.development && authority.practiceDevelopment) fail("OWNER_THEORY_DEVELOPMENT_BINDING_MISMATCH");
  const approval = practice ? await readPracticeDevelopmentApproval(root, settings) : await readTheoryDevelopmentApproval(root, settings);
  const submittedApproval = practice ? authority.practiceDevelopment : authority.development;
  if (authority.userId !== approval.userId || authority.questionSha256 !== approval.questionSha256 ||
      submittedApproval?.supabaseUrl !== approval.supabaseUrl || submittedApproval?.approvalId !== approval.approvalId ||
  ...
  try {
    const budget = await readTheoryBudget(root, settings);
    if (budget.connectionPending || (budget.caseId === null && budget.usedReservations === 0)) fail("OWNER_THEORY_CONNECTION_NOT_READY");
    const rows = await developmentReservations(root, scope);
    ...
    const maximumDevelopmentCalls = practice ? await readPracticeDevelopmentCallLimit(root, settings) : await readTheoryDevelopmentCallLimit(root, settings);
    ...
    for (let slot = 1; slot <= THEORY_POLICY.maximumCalls; slot++) {
      try {
        await durableCreate(path.join(root, `call-${String(slot).padStart(2, "0")}.json`), { slot, purpose: authority.purpose,
          scope, approvalId: approval.approvalId, userId: approval.userId, sourceItemId: authority.sourceItemId,
...
export async function generateOwnerTheory(root, settings, authority, request, transport = fetch) {
  ...
  const slot = authority.practiceDevelopment ? await reservePracticeDevelopmentCall(root, settings, authority) : authority.development ? await reserveTheoryDevelopmentCall(root, settings, authority) : await reserveTheoryCall(root, settings, authority);
...
export const PRACTICE_DEVELOPMENT_APPROVAL_ID = "owner-practice-development-20260917";
function practiceApprovalFile(root) { return path.join(path.dirname(root), "practice-development-approval-20260917.json"); }
function validatePracticeApproval(value, settings) {
  if (!value || Object.keys(value).sort().join(",") !== "approvalId,installationSha256,maximumCalls,maximumReservationMicros,maximumSourceItems,questionSha256,supabaseUrl,userId" ||
      value.approvalId !== PRACTICE_DEVELOPMENT_APPROVAL_ID || value.userId !== "aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa" || value.userId === settings.ownerId ||
      value.supabaseUrl !== "http://127.0.0.1:55431" || !/^[a-f0-9]{64}$/.test(value.questionSha256 ?? "") ||
      !Number.isInteger(value.maximumCalls) || value.maximumCalls < 1 || value.maximumCalls > 4 ||
      value.maximumSourceItems !== 3 || value.maximumReservationMicros !== value.maximumCalls * THEORY_POLICY.reservationMicros ||
      value.installationSha256 !== digest(JSON.stringify(binding(settings)))) fail("OWNER_THEORY_PRACTICE_APPROVAL_REQUIRED");
  return value;
}
// Explicit installation only after the separate Owner approval. No budget reset or new ledger.
export async function authorizePracticeDevelopment(root, settings, input) {
  validateTheorySettings(settings);
  const approval = validatePracticeApproval({...input, approvalId:PRACTICE_DEVELOPMENT_APPROVAL_ID,
    maximumSourceItems:3, maximumReservationMicros:input.maximumCalls * THEORY_POLICY.reservationMicros,
    installationSha256:digest(JSON.stringify(binding(settings)))}, settings);
  const shared = await readTheoryBudget(root, settings);
  if (shared.connectionPending || shared.developmentUsedCalls !== 7 || shared.remainingMicros < approval.maximumReservationMicros)
    fail("OWNER_THEORY_PRACTICE_NOT_READY");
  await durableCreate(practiceApprovalFile(root), approval);
}
export async function readPracticeDevelopmentApproval(root, settings) {
  try { return validatePracticeApproval(JSON.parse(await readFile(practiceApprovalFile(root), "utf8")), settings); }
  catch(error) { if(error.code === "ENOENT") fail("OWNER_THEORY_PRACTICE_APPROVAL_REQUIRED"); throw error; }
}
export async function readPracticeDevelopmentUsage(root, settings) {
  const approval = await readPracticeDevelopmentApproval(root, settings);
  const rows = await developmentReservations(root, "practice_development");
  if (rows.some(row => row.approvalId !== approval.approvalId || row.userId !== approval.userId))
    fail("OWNER_THEORY_DEVELOPMENT_BINDING_MISMATCH");
  return {usedCalls:rows.length, maximumCalls:await readPracticeDevelopmentCallLimit(root, settings)};
}
export async function reservePracticeDevelopmentCall(root, settings, authority) {
  return reserveDevelopmentCall(root, settings, authority, "practice_development");
}

// One later Owner amendment; never changes the original four-call approval or ledger.
function practiceExtensionFile(root) { return path.join(path.dirname(root), "practice-call-extension-20260917.json"); }
function practiceExtension(approval) {
  return { approvalId: "owner-practice-development-extra-call-20260917", practiceApprovalSha256: digest(JSON.stringify(approval)),
    maximumPracticeCalls: 5, additionalReservationMicros: THEORY_POLICY.reservationMicros };
}
export async function readPracticeDevelopmentCallLimit(root, settings) {
  const approval = await readPracticeDevelopmentApproval(root, settings);
  let extension;
  try { extension = JSON.parse(await readFile(practiceExtensionFile(root), "utf8")); }
  catch (error) { if (error.code === "ENOENT") return approval.maximumCalls; fail("OWNER_THEORY_PRACTICE_EXTENSION_INVALID"); }
  if (approval.maximumCalls !== 4 || JSON.stringify(extension) !== JSON.stringify(practiceExtension(approval)))
    fail("OWNER_THEORY_PRACTICE_EXTENSION_INVALID");
  return extension.maximumPracticeCalls;
}
export async function authorizeAdditionalPracticeDevelopmentCall(root, settings) {
  validateTheorySettings(settings);
  const approval = await readPracticeDevelopmentApproval(root, settings);
  const budget = await readTheoryBudget(root, settings);
  const usage = await readPracticeDevelopmentUsage(root, settings);
  if (approval.maximumCalls !== 4 || budget.connectionPending || budget.developmentUsedCalls !== 7 ||
      usage.usedCalls < 3 || usage.usedCalls > 4 ||
      budget.remainingMicros < (5 - usage.usedCalls) * THEORY_POLICY.reservationMicros)
    fail("OWNER_THEORY_PRACTICE_EXTENSION_NOT_READY");
  await durableCreate(practiceExtensionFile(root), practiceExtension(approval));
}
