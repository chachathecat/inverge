...
type AnswerReviewStructureInput = {
  ...
  explanationLevel?: AnswerReviewExplanationLevel;
};
        ...
        parts: [
          {
            text: [
              ...
              "explanationLevel에 맞춰 해설 난이도를 조절한다.",
              "easy: 중고등학생도 이해할 수 있게 설명한다.",
              "easy: 전문용어는 괄호 안에 쉬운 말로 풀어쓴다.",
              "easy: 계산/법리/논점을 작은 단계로 나눈다.",
              "easy: 단, 법리·산식·필수 키워드를 왜곡하지 않는다.",
              "standard: 일반 수험생용으로 핵심 이유와 적용 순서를 설명한다.",
              "exam: 문장형 모범답안을 쓰지 않는다.",
              "exam: 목차와 필수 키워드, 문단 보강 포인트 중심으로 쓴다.",
              ...
              "문제에서 요구한 내용의 의미가 답안에 이미 있으면 표현·순서·동의어 차이를 누락으로 지적하지 않는다. 명시적 정의를 요구하지 않은 문제에서 사례 적용이 충분하면 별도 정의 문장의 부재를 약점으로 만들지 않는다. 누락이라고 판단하기 전에 답안 전체에서 같은 뜻의 설명과 사례 적용이 있는지 확인한다.",
...
function answerReviewStructureSchema(): Schema {
  return {
    ...
    properties: {
      ...
      answerEvidenceQuote: { type: SchemaType.STRING },
      ...
      keyTermExplanations: { type: SchemaType.ARRAY, items: { type: SchemaType.STRING } },
    ...
    required: ["diagnosticStatus", "questionRequirementQuote", "answerEvidenceQuote", "reviewedAnswerScope",
      "questionSummary",
      "coreConcepts",
      "requiredIssues",
      "userAnswerSummary",
