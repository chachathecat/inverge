...
export function validateMetaAuditBundle(
  ...
  const baselineBindings = new Set<string>();
  const currentBindings = new Set<string>();
  const baselineIdentityDigests = new Set<string>();
  const currentIdentityDigests = new Set<string>();
  const baselineProviderVersions = new Set<string>();
  const currentProviderVersions = new Set<string>();
  const baselineArtifacts = new Set<string>();
  const currentArtifacts = new Set<string>();
  const driftExecutionIds = new Set<string>();
  let disagreements = 0;
  if (isRecord(drift) && Array.isArray(drift.fixtures)) {
    ...
    for (const [index, fixture] of drift.fixtures.entries()) {
      ...
      for (const [kind, outcome] of [["baseline", fixture.baseline], ["current", fixture.current]] as const) {
        ...
        if (modelValid && isSafeId(outcome.judgeId) && isSafeId(outcome.judgeVersion)) {
          const identity = judgeModelIdentity as QuestionFoundryModelIdentityV1;
          const bindingDigest = canonicalDigest({
            judgeId: outcome.judgeId,
            judgeVersion: outcome.judgeVersion,
            providerId: identity.providerId,
            modelFamilyId: identity.modelFamilyId,
            modelVersionId: identity.modelVersionId,
            modelArtifactDigest: identity.modelArtifactDigest,
          });
          const identityDigest = canonicalDigest(identity);
          const providerVersion = canonicalDigest([identity.providerId, identity.modelVersionId]);
          const bindings = kind === "baseline" ? baselineBindings : currentBindings;
          const identities = kind === "baseline" ? baselineIdentityDigests : currentIdentityDigests;
          const providerVersions = kind === "baseline"
            ? baselineProviderVersions
            : currentProviderVersions;
          const artifacts = kind === "baseline" ? baselineArtifacts : currentArtifacts;
          bindings.add(bindingDigest);
          identities.add(identityDigest);
          providerVersions.add(providerVersion);
          artifacts.add(identity.modelArtifactDigest);
        }
        if (
          !isSafeId(outcome.judgeExecutionId) ||
          driftExecutionIds.has(String(outcome.judgeExecutionId))
        ) {
          driftFixturesValid = false;
        }
        driftExecutionIds.add(String(outcome.judgeExecutionId));
      }
      if (
  ...
  if (
    baselineBindings.size !== 1 ||
    currentBindings.size !== 1 ||
    baselineIdentityDigests.size !== 1 ||
    currentIdentityDigests.size !== 1 ||
    baselineProviderVersions.size !== 1 ||
    currentProviderVersions.size !== 1 ||
    baselineArtifacts.size !== 1 ||
    currentArtifacts.size !== 1 ||
    [...baselineIdentityDigests][0] === [...currentIdentityDigests][0] ||
    [...baselineProviderVersions][0] === [...currentProviderVersions][0] ||
    [...baselineArtifacts][0] === [...currentArtifacts][0]
  ) {
    driftFixturesValid = false;
  }
