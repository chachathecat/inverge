...
function validateSealedTransferVariantContent(
  bundle: ReleaseEvidenceBundleV1,
  trustContext: ReleaseTrustContextV1,
  variant: SealedTransferVariantV1,
  sourceCandidate: QuestionCandidateV1,
  errors: string[],
) {
  const projectedCandidate: QuestionCandidateV1 = {
    ...sourceCandidate,
    candidateId: variant.transferVariantId,
    stem: variant.stem,
    options: variant.options,
    proposedCorrectOptionId: variant.expectedOptionId,
    sourceBindingDigest: variant.sourceBindingDigest,
    rightsBoundary: variant.rightsBoundary,
  };
  const expectedAnswer = normalizeQuestionText(bundle.batch.answerSpecification.expectedAnswer);
  const correct = projectedCandidate.options.find(
    (option) => option.optionId === projectedCandidate.proposedCorrectOptionId,
  );
  const matchingExpectedAnswers = projectedCandidate.options.filter(
    (option) => normalizeQuestionText(option.body) === expectedAnswer,
  );
  if (
    !correct ||
    ...
    errors.push(`SEALED_TRANSFER_VARIANT_SINGLE_ANSWER_INVALID:${variant.transferVariantId}`);
  }
  for (const cause of validateCandidateCalculation(
    projectedCandidate,
    bundle.batch.answerSpecification,
  ).errors) {
    errors.push(
      `SEALED_TRANSFER_VARIANT_CALCULATION_INVALID:${variant.transferVariantId}:${cause}`,
    );
  }
  for (const cause of reviewDistractorsAndAnswerClues(
    projectedCandidate,
    bundle.batch.blueprint,
  ).errors) {
    errors.push(
      `SEALED_TRANSFER_VARIANT_DISTRACTOR_OR_CLUE_INVALID:${variant.transferVariantId}:${cause}`,
    );
  }
  const normalizedVariantBody = normalizeQuestionText(
    `${variant.stem} ${variant.options.map((option) => option.body).join(" ")}`,
  );
  for (const forbidden of bundle.batch.answerSpecification.forbiddenAnswerPatterns) {
    const pattern = normalizeQuestionText(forbidden);
    if (pattern && normalizedVariantBody.includes(pattern)) {
      errors.push(
        `SEALED_TRANSFER_VARIANT_DISTRACTOR_OR_CLUE_INVALID:${variant.transferVariantId}:FORBIDDEN_ANSWER_PATTERN:${forbidden}`,
      );
    }
  }
  try {
    const review = buildSimilarityFirewallReview(
      projectedCandidate,
      bundle.similarityReferences,
      bundle.trustedSources,
      trustContext.sourceRegistryExportBinding,
    ...
    if (
      review.candidateId !== variant.transferVariantId ||
      bundle.similarityReferences.length < 1 ||
      review.referenceCount !== bundle.similarityReferences.length ||
      review.corpusDigest !== bundle.similarityReview.corpusDigest ||
      review.threshold !== QUESTION_FOUNDRY_SIMILARITY_THRESHOLD ||
      !Number.isFinite(review.maximumTokenJaccard) ||
      review.maximumTokenJaccard >= QUESTION_FOUNDRY_SIMILARITY_THRESHOLD ||
      review.nearCopyDetected ||
      review.reconstructionRiskDetected ||
      ...
      errors.push(`SEALED_TRANSFER_VARIANT_SIMILARITY_INVALID:${variant.transferVariantId}`);
    }
  } catch {
    errors.push(`SEALED_TRANSFER_VARIANT_SIMILARITY_INVALID:${variant.transferVariantId}`);
  }
...
function validateTransferEvidence(
  ...
  for (const variant of variants) {
    ...
    if (candidate) {
      validateSealedTransferVariantContent(bundle, trustContext, variant, candidate, errors);
    }
  }
  const uniqueFields = [
