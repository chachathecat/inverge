...
import {
  ...
  isConceptNodeCandidate,
} from "@/lib/review-os/concept-node-mapping";
...
} from "@/lib/review-os/mode-migration";
import {
  getKstDayKey,
  isSameKstDay,
  isOverdueDueAt,
...
function isSmokeSeedItem(
  item: Pick<
    WrongAnswerItemRecord,
    "sourceLabel" | "problemTitle" | "rawQuestionText" | "rawAnswerText"
  >,
) {
  return (
...
function isSmokeSeedQueueItem(
  item: Pick<
    ReviewQueueCard,
    "subjectLabel" | "problemTitle" | "reviewReason" | "topicTag"
  >,
) {
  return (
...
function isSmokeSeedStudyLog(log: {
  sourceLabel: string;
  notUnderstood: string;
  revisitNeeded: string;
}) {
  return (
    containsSmokeSeedText(log.sourceLabel) ||
    containsSmokeSeedText(log.notUnderstood) ||
    containsSmokeSeedText(log.revisitNeeded)
  );
}
...
function isSmokeSeedWeeklySummary(summary: WeeklyLearningSummaryRecord | null) {
  ...
  return [
    ...summary.topMistakeTypes,
    ...summary.topTopics,
    ...summary.nextWeekFocus,
  ].some((value) => containsSmokeSeedText(value));
}
...
function rankQueueItem(item: {
  ...
  const confidenceScore =
    item.confidence === "낮음" ? 16 : item.confidence === "중간" ? 8 : 0;
  ...
  const recencyScore = item.createdAt
    ? Math.max(
        0,
        20 -
          Math.floor((Date.now() - Date.parse(item.createdAt)) / 86_400_000) *
            4,
      )
    : 0;
...
function getReviewReason(item: {
  ...
  if (item.recurrenceCount >= 3)
    return "같은 유형이 세 번 이상 반복된 항목입니다.";
  if (item.confidence === "낮음")
    return "확신 없이 맞히거나 틀린 항목이라 다시 볼 가치가 있습니다.";
...
function getFocusMode(
  ...
  const source = [
    ...queue.map((item) => item.examName),
    ...recentItems.map((item) => item.examName),
  ];
...
function resolveNextActionType(
  item: ReviewQueueCard,
  mode: "first" | "second",
): TodayFocus["nextActionType"] {
  const rewriteSignal =
    /rewrite|재작성|문단/.test(item.reviewReason) ||
    /구조|문단|논점 누락/.test(item.mistakeType);
  if (mode === "second" && rewriteSignal) return "rewrite_now";
  if (mode === "first" && /재시도|retry/.test(item.reviewReason))
    return "retry_now";
  return "review_now";
}
...
function buildTodayPriorityPlan(
  ...
  const scored = queue.map((item) => {
    ...
    const rewriteRetryUrgency =
      nextActionType === "rewrite_now" || nextActionType === "retry_now"
        ? 14
        : 0;
    const modeScore =
      mode === "second" && nextActionType === "rewrite_now" ? 8 : 4;
    const score =
      dueScore +
      recurrenceScore +
      confidenceScore +
  ...
  const estimatedDurationMinutes = Math.max(
    ...
    Math.min(
      35,
      Math.round(
        ((top.item.timeSpentSeconds ??
          (mode === "second" ? 22 * 60 : 15 * 60)) *
          0.8) /
          60,
      ),
    ),
...
function makeTodayFocus(
  ...
  const staleCount = queue.filter(
    (item) => Date.now() - Date.parse(item.dueAt) > 2 * 86_400_000,
  ).length;
  const mistakeLine =
    mode === "second"
      ? `먼저 볼 보강 지점은 ${topMistake}입니다.`
      : `오늘 줄일 실수는 ${topMistake}입니다.`;
  const staleLine =
    staleCount > 0
      ? `밀린 다시 볼 항목 ${staleCount}개 중 1개만 먼저 처리합니다.`
      : "한 번에 하나씩 실행하고 다음 항목으로 넘어갑니다.";
...
function makeWeeklySummaryText(
  topMistakes: string[],
  topTopics: string[],
  slowCount: number,
...
function resolveWeeklyTaskAction(
  queueItem: ReviewQueueCard,
  mode: "first" | "second",
): WeeklyPlanTaskAction {
  const rewriteSignal =
    /rewrite|재작성|문단/.test(queueItem.reviewReason) ||
    /구조|문단|논점 누락|누락/.test(queueItem.mistakeType);
  if (mode === "second" && rewriteSignal) return "rewrite";
  if (mode === "first" && /재시도|retry/.test(queueItem.reviewReason))
    return "retry";
  return "review";
}

function buildWeeklyTaskReason(
  queueItem: ReviewQueueCard,
  overdueDays: number,
) {
  if (overdueDays > 0) return `예정 복습 시점에서 ${overdueDays}일 지났습니다.`;
  if (queueItem.recurrenceCount >= 3)
    return "반복된 실수가 누적되어 이번 주에 먼저 줄여야 합니다.";
  if (queueItem.confidence === "낮음")
    return "확신이 낮았던 항목이라 근거를 다시 고정해야 합니다.";
  if ((queueItem.timeSpentSeconds ?? 0) >= 180)
    return "풀이 시간이 길어져 처리 순서 점검이 필요합니다.";
...
function buildWeeklyTaskDuration(
  queueItem: ReviewQueueCard,
  action: WeeklyPlanTaskAction,
  mode: "first" | "second",
) {
  const baselineSeconds =
    queueItem.timeSpentSeconds ?? (mode === "second" ? 22 * 60 : 15 * 60);
  const multiplier =
    action === "rewrite" ? 1.0 : action === "retry" ? 0.9 : 0.75;
...
function buildWeeklyPlan(
  ...
  const now = new Date();
  const overdueQueue = queue.filter(
    (item) => Date.parse(item.dueAt) < now.getTime(),
  );
  const recentWindow = recentItems.filter(
    (item) => now.getTime() - Date.parse(item.createdAt) <= 14 * 86_400_000,
  );
  const sorted = queue
    .map((item) => {
      const dueScore = getDueScore(item.dueAt, now);
      ...
      const overdueDays = Math.max(
        0,
        Math.floor((now.getTime() - dueTs) / 86_400_000),
      );
  ...
  const tasks: WeeklyPlanTask[] = sorted.slice(0, 3).map((entry, index) => {
    ...
    return {
      ...
      estimatedDurationMinutes: buildWeeklyTaskDuration(
        entry.item,
        action,
        mode,
      ),
  ...
  const recoveryCandidateEntry =
    sorted.find((entry) => entry.overdueDays >= 2) ??
    sorted.find((entry) => entry.overdueDays >= 1) ??
    null;
...
function isMeaningfullyCorrectAnswer(
...
export class ReviewOsService {
  ...
  async completeCalculatorRoutine(
    userId: string,
    email: string | null,
    input: unknown,
  ) {
    ...
    const { status, record } =
      await reviewOsRepository.createLearningSignalEventWithId(
        userId,
    ...
    if (conceptStateResult.status === "updated") {
      await reviewOsRepository.logUsageEvent(
        userId,
        "calculator_routine_concept_state_updated",
    ...
    await reviewOsRepository.logUsageEvent(
      ...
      record.id,
      {
        ...
        nextPlanCandidateTypes: nextPlanSignal.candidates.map(
          (candidate) => candidate.taskType,
        ),
      },
    );
    return {
  ...
  async listCalculatorRoutineReviewCandidates(
    userId: string,
    email: string | null,
    limit = 3,
  ) {
    await this.ensureAccess(userId, email);
    const events = await reviewOsRepository.listLearningSignalEvents(
      userId,
      "second",
      80,
    );
    return buildActiveCalculatorRoutineReviewCandidates(
      events,
      new Date(),
      limit,
    );
  ...
  async countLearningSignalEvents(
    userId: string,
    email: string | null,
    mode: AppraisalMode,
  ) {
  ...
  async getLearningSignalSummary(
    userId: string,
    email: string | null,
    mode: AppraisalMode,
    ...
    const totalCount = await this.countLearningSignalEvents(
      userId,
      email,
      mode,
    );
    ...
    for (const event of events) {
      subjectCount.set(
        event.subject,
        (subjectCount.get(event.subject) ?? 0) + 1,
      );
      nextTaskTypeCount.set(
        event.nextTaskType,
        (nextTaskTypeCount.get(event.nextTaskType) ?? 0) + 1,
      );
      for (const tag of event.derivedTags)
    ...
    return {
      ...
      nextTaskTypes: rank(nextTaskTypeCount).map(([type, count]) => ({
        type,
        count,
      })),
    };
  ...
  private resolveFollowUpDueAtFromAction(
    ...
    if (existingNextReviewDate) {
      return resolveScheduleOverrideDate(
        existingNextReviewDate,
        addDays(examName === "감정평가사 2차" ? 2 : 3),
      );
    }
  ...
  private getCompletionReviewReason(
    action: ReviewCompletionAction,
    examName: string,
  ) {
    if (action === "first_short_retry")
  ...
  private isCompletionActionCompatible(
    ...
    if (examName === "감정평가사 2차") {
      return (
        ...
        action === "second_keep_scheduled_rewrite"
      );
    }
    return (
  ...
  private readPreservedNextReviewDate(
    context: Awaited<
      ReturnType<typeof reviewOsRepository.getReviewQueueItemContext>
    >,
  ) {
    ...
    const queueRawPayload =
      typeof context.queueRow.raw_payload === "object" &&
      context.queueRow.raw_payload
    ...
    const normalizedDraft =
      itemRawPayload?.normalized_draft &&
      typeof itemRawPayload.normalized_draft === "object"
        ? (itemRawPayload.normalized_draft as Record<string, unknown>)
        : null;

    const candidates = [
      context.item.nextReviewDate,
      typeof itemRawPayload?.nextReviewDate === "string"
        ? itemRawPayload.nextReviewDate
        : null,
      typeof normalizedDraft?.nextReviewDate === "string"
        ? normalizedDraft.nextReviewDate
        : null,
      typeof userConfirmedFields?.nextReviewDate === "string"
        ? userConfirmedFields.nextReviewDate
        : null,
  ...
  async upsertStudyProfile(
    ...
    const normalizedInput = {
      ...
      preferredSubjects: normalizePreferredSubjectsForMode(
        input.preferredSubjects,
        mode,
      ),
    };
    const profile = await reviewOsRepository.upsertStudyProfile(
      userId,
      normalizedInput,
    );
    await reviewOsRepository.logUsageEvent(
  ...
  private async buildUsageSummary(
    ...
    const monthlyUsed = await reviewOsRepository.countMonthlyWrongAnswers(
      userId,
      getMonthStartIso(),
    );
    return {
  ...
  async createWrongAnswerItem(
    ...
    try {
      ...
      const captureSignals = isCaptureCreated
        ? buildCaptureNoteSignals(mode, normalizedInput)
        : null;
      ...
      if (isCaptureCreated) {
        try {
          captureSignalsV2 = structureCaptureNote({
            ...
            subject: normalizedInput.subjectLabel,
            confirmedText:
              normalizedInput.rawQuestionText ?? normalizedInput.userAnswer,
            problemText: normalizedInput.rawQuestionText,
            userAnswerText: normalizedInput.userAnswer,
            existingNormalizedDraft:
              normalizedInput.extractionPayload?.normalized_draft ?? null,
            userConfirmedFields:
      ...
      try {
        ...
        console.warn(
          "[review-os] wrong answer taxonomy classification fallback",
          error,
        );
      }
      ...
      const secondRewriteSignal =
        mode === "second"
          ? buildSecondAnswerRewriteSignal(normalizedInput)
          : null;
      const taxonomyNodeLabel =
        taxonomyClassification?.candidates[0]?.topic ??
        taxonomyClassification?.primaryNodeId ??
        null;
      const conceptNodeCandidate = buildConceptNodeCandidate({
        ...
        metadata: {
          ...
          nextAction:
            ...
            input.biggestGap,
          calculationRisk:
            input.calculationRisk ?? secondRewriteSignal?.calculationRisk,
          unitRisk: input.unitRisk ?? secondRewriteSignal?.unitRisk,
          supportedCalculatorTemplateId:
            input.supportedCalculatorTemplateId ??
            secondRewriteSignal?.supportedCalculatorTemplateId,
          taxonomy_node_label: taxonomyNodeLabel,
        },
      });
      const answerSubmissionConfirmedFields: Record<string, unknown> =
        input.extractionPayload?.user_confirmed_fields &&
        typeof input.extractionPayload.user_confirmed_fields === "object"
          ? input.extractionPayload.user_confirmed_fields
          : {};
      const confirmedPageCount =
        typeof answerSubmissionConfirmedFields.pageCount === "number" &&
        Number.isFinite(answerSubmissionConfirmedFields.pageCount)
      ...
      const answerSubmissionContract = isCaptureCreated
        ? buildLearnerAnswerSubmissionPersistenceContract({
            ...
            lowConfidenceFlag:
              answerSubmissionConfirmedFields.lowConfidenceFlag === true,
            captureQualityIssue:
              typeof answerSubmissionConfirmedFields.captureQualityIssue ===
              "string"
                ? answerSubmissionConfirmedFields.captureQualityIssue
                : null,
            hasManualCorrection:
              answerSubmissionConfirmedFields.hasManualCorrection === true,
            ocrConfirmedByLearner:
              answerSubmissionConfirmedFields.ocrConfirmedByLearner === true,
            confirmedText:
              normalizedInput.rawQuestionText ?? normalizedInput.userAnswer,
          })
        : null;
      ...
      const item = await reviewOsRepository.insertWrongAnswerItem(
        ...
        normalizedInput,
        {
          ...
          raw_ocr_text:
            input.extractionPayload?.raw_ocr_text ??
            input.rawQuestionText ??
            null,
          raw_extraction_json:
            input.extractionPayload?.raw_extraction_json ?? {},
          normalized_draft: input.extractionPayload?.normalized_draft ?? null,
          user_confirmed_fields: input.extractionPayload
          ...
          aiDraft: {
            ...
            calculationRisk:
              input.calculationRisk ??
              secondRewriteSignal?.calculationRisk ??
              null,
        ...
        },
        {
          ...
          review_stage: input.conceptCard?.reviewStage ?? null,
          cloze_candidate:
            input.conceptCard?.trapWords?.[0] ?? input.keyConcepts?.[0] ?? null,
          rewriteTaskType: secondRewriteSignal?.rewriteTaskType ?? null,
          missingIssueCandidate:
            secondRewriteSignal?.missingIssueCandidate ?? null,
      ...
      await reviewOsRepository.insertWrongAnswerTag(
        userId,
        item.id,
        artifacts.tags,
      );
      ...
      await reviewOsRepository.insertReviewQueueEntry(
        userId,
        item,
        reviewReason,
        priorityScore,
        ...
        try {
          await reviewOsRepository.createLearningSignalEvent(
            ...
            buildCaptureLearningSignal({
              ...
              calculationRisk:
                secondRewriteSignal?.calculationRisk ?? undefined,
          ...
          console.warn(
            "[review-os] capture learning signal event skipped",
            error,
          );
        }
      }
  ...
  getWrongAnswerDetail(
    ...
    return this.ensureAccess(userId, email).then(() =>
      reviewOsRepository.getWrongAnswerDetail(userId, itemId),
    );
  }
  ...
  async getReviewQueue(userId: string, email: string | null) {
    ...
    const getRecentCaptureBoost = (queueItem: (typeof queue)[number]) => {
      ...
      if (Number.isFinite(parsedCreatedAt)) {
        return now >= parsedCreatedAt &&
          now - parsedCreatedAt <= recentCaptureWindowMs
          ? 15
          : 0;
      }
      const parsedDueAt = Date.parse(queueItem.dueAt);
      return Number.isFinite(parsedDueAt) &&
        now >= parsedDueAt &&
        now - parsedDueAt <= recentCaptureWindowMs
        ? 15
        : 0;
    };
  ...
  async completeReview(
    ...
    await this.ensureAccess(userId, email);
    const context = await reviewOsRepository.getReviewQueueItemContext(
      userId,
      queueId,
    );
    if (
    ...
    if (context) {
      const derivedPayload =
        typeof context.queueRow.derived_payload === "object" &&
        context.queueRow.derived_payload
      ...
      const recurrenceCount =
        typeof derivedPayload.recurrenceCount === "number"
          ? derivedPayload.recurrenceCount
          : 1;
      ...
      const rawRewriteParagraph =
        ...
        typeof metadata.rewriteParagraph === "string"
          ...
          : "";
      const conceptNodeCandidate =
        readConceptNodeCandidateFromPayload(derivedPayload);
      await reviewOsRepository.createFollowUpReviewQueueEntry(
        ...
        followUpPriority,
        {
          ...
          rewriteInstruction:
            ...
            context.item.rewriteInstruction ??
            null,
        },
        rawRewriteParagraph
          ? {
    ...
    const metricConceptNodeCandidate = context
      ? readConceptNodeCandidateFromPayload(context.queueRow.derived_payload)
      : null;
    recordLearningMetricIfEnabled(
      buildLearningMetricEvent({
        eventName: "review_queue_task_completed",
        examMode:
          context?.item.examName === "감정평가사 2차" ? "second" : "first",
        subject: context?.item.subjectLabel,
        conceptNodeId:
          metricConceptNodeCandidate?.conceptNodeId ??
          context?.primaryTag?.topicTag ??
          context?.item.subjectLabel,
        taskType: action,
  ...
  async migrateFirstToSecondMode(userId: string, email: string | null) {
    ...
    const firstQueue = rawQueue.filter(
      (item) =>
        item.examName === "감정평가사 1차" && !isSmokeSeedQueueItem(item),
    );
    const snapshot = buildFirstToSecondMigrationSnapshot({
      firstItems,
      firstQueue,
      firstLearningSignals: firstSignals,
    });
    await reviewOsRepository.archiveReviewQueueItemsForMode(
      userId,
      firstQueue.map((item) => item.queueId),
    );
    const preferredSubjects = normalizePreferredSubjectsForMode(
    ...
    await reviewOsRepository.logUsageEvent(
      userId,
      "first_to_second_mode_migration_confirmed",
      "study_profile",
      userId,
      {
        activeMode: "second",
  ...
  async getTodayFocus(
    ...
    const queue = targetExamName
      ? rawQueue.filter((item) => item.examName === targetExamName)
      : rawQueue;
    ...
    await reviewOsRepository.insertActionSeed(userId, {
      ...
      rawPayload: {
        nextActionType: focus.nextActionType,
        sourceQueueId: focus.sourceQueueId,
        sourceItemId: focus.sourceItemId,
      },
    });
      ...
      rawPayload: {
        ...
        recovery: plan.recovery
          ? {
              ...
              estimatedDurationMinutes:
                plan.recovery.task.estimatedDurationMinutes,
            }
          : null,
      },
    });
  ...
  async getWeeklySummary(
    ...
    const details = await Promise.all(
      recentWeekItems
        .slice(0, 20)
        .map((item) =>
          reviewOsRepository.getWrongAnswerDetail(userId, item.id),
        ),
    );
    const tags = details.flatMap((detail) =>
      detail?.tags[0] ? [detail.tags[0]] : [],
    );
    ...
    const topTopics = [...topicCounts.entries()]
      ...
      .slice(0, 3)
      .map(([key]) => key);
    const slowCount = recentWeekItems.filter(
      (item) => (item.timeSpentSeconds ?? 0) >= 180,
    ).length;
    const lowConfidenceCount = recentWeekItems.filter(
      (item) => item.confidence === "낮음",
    ).length;
    const nextWeekFocus = [
      topMistakeTypes[0]
        ? `${topMistakeTypes[0]}부터 다시 정리하기`
        : "반복 실수부터 다시 정리하기",
      topTopics[0]
        ? `${topTopics[0]} 관련 항목 먼저 보기`
        : "최근 항목 먼저 보기",
      slowCount > 0
        ? "시간이 오래 걸린 문제의 풀이 순서 줄이기"
        : "확신 낮은 항목의 근거 문장 다시 보기",
    ];
    const summaryText = makeWeeklySummaryText(
      topMistakeTypes,
      topTopics,
      slowCount,
      lowConfidenceCount,
  ...
  async createStudyLog(
    ...
    const log = await reviewOsRepository.createStudyLog(
      userId,
      normalizedInput,
      {
  ...
  async listStudyLogs(
    ...
    await this.ensureAccess(userId, email);
    const logs = await reviewOsRepository.listStudyLogs(
      userId,
      mode,
  ...
  async getDailyStudyActivity(
    ...
    const dayStartUtcIso = new Date(
      `${kstDayKey}T00:00:00+09:00`,
    ).toISOString();
    const yesterdayStartUtcIso = new Date(
      `${yesterdayKstDayKey}T00:00:00+09:00`,
    ).toISOString();
    ...
    const savedCaptureToday =
      ...
      recentUsageEvents.some(
        (event) =>
          event.eventName === "capture_saved" &&
          isSameKstDay(event.createdAt, now),
      );
    const startedExecutionToday = recentUsageEvents.some(
      (event) =>
        event.eventName === "post_save_execution_started" &&
        isSameKstDay(event.createdAt, now),
    );
    const completedTodayTask = recentUsageEvents.some(
      (event) =>
        (event.eventName === "post_save_execution_completed" ||
          event.eventName === "review_complete") &&
        isSameKstDay(event.createdAt, now),
    );
    const followupScheduledToday = recentUsageEvents.some(
      (event) =>
        event.eventName === "review_followup_scheduled" &&
        isSameKstDay(event.createdAt, now),
    );
    const recoveredOverdueToday = recentUsageEvents.some(
      (event) =>
        event.eventName === "review_complete" &&
        isSameKstDay(event.createdAt, now),
    );
  ...
  async hasMeaningfulLearningData(
    userId: string,
    email: string | null,
    mode: "first" | "second",
  ) {
  ...
  async getAdminBetaFunnel(): Promise<AdminBetaFunnel> {
    ...
    const breakdowns = Object.fromEntries(
      breakdownKeys.map((key) => [key, new Map<string, number>()]),
    ) as Record<(typeof breakdownKeys)[number], Map<string, number>>;

    for (const event of events) {
      countByEvent.set(
        event.eventName,
        (countByEvent.get(event.eventName) ?? 0) + 1,
      );
    ...
    const toFunnel = (steps: readonly string[]) =>
      steps.map((eventName, index) => {
        ...
        const previousCount =
          index > 0 ? (countByEvent.get(steps[index - 1]) ?? 0) : 0;
    ...
    const friction = [
      {
        ...
        rate: safeRate(
          countByEvent.get("ocr_failed") ?? 0,
          countByEvent.get("capture_started") ?? 0,
        ),
      },
      {
        ...
        rate: safeRate(
          countByEvent.get("pdf_manual_fallback") ?? 0,
          countByEvent.get("capture_started") ?? 0,
        ),
      },
      {
        ...
        count: Math.max(
          0,
          (countByEvent.get("capture_saved") ?? 0) -
            (countByEvent.get("post_save_execution_started") ?? 0),
        ),
        rate: safeRate(
          Math.max(
            0,
      ...
      },
      {
        ...
        count: Math.max(
          0,
          (countByEvent.get("post_save_execution_started") ?? 0) -
            (countByEvent.get("post_save_execution_completed") ?? 0),
        ),
        rate: safeRate(
          Math.max(
            0,
      ...
      },
      {
        ...
        count: Math.max(
          0,
          (countByEvent.get("overdue_recovery_shown") ?? 0) -
            (countByEvent.get("overdue_recovery_started") ?? 0),
        ),
        rate: safeRate(
          Math.max(
            0,
    ...
    for (const event of events) {
      ...
      if (
        day === nowDay &&
        (event.metadataJson.dailyState === "studiedToday" ||
          event.eventName === "today_task_completed")
      ) {
    ...
    const dayDiff = (a: string, b: string) =>
      Math.floor(
        (Date.parse(`${b}T00:00:00+09:00`) -
          Date.parse(`${a}T00:00:00+09:00`)) /
          86_400_000,
      );
    ...
    const cohortAnalytics = [...cohortMap.entries()]
      ...
      .map(([cohortDate, users]) => {
        ...
        const d1 = users.filter((u) =>
          [...u.eventDays].some((day) => dayDiff(cohortDate, day) === 1),
        ).length;
        const d3 = users.filter((u) =>
          [...u.eventDays].some((day) => dayDiff(cohortDate, day) === 3),
        ).length;
        ...
        const d7 = maturedForD7
          ? users.filter((u) =>
              [...u.eventDays].some((day) => dayDiff(cohortDate, day) === 7),
            ).length
          : 0;
        const firstCaptureSaved = users.filter(
          (u) => u.counts.capture_saved > 0,
        ).length;
        const firstExecutionCompleted = users.filter(
          (u) => u.counts.post_save_execution_completed > 0,
        ).length;
        const firstFollowupScheduled = users.filter(
          (u) => u.counts.review_followup_scheduled > 0,
        ).length;
        const captureUsers = users.filter((u) => u.counts.capture_saved > 0);
        const executionStartedUsers = users.filter(
          (u) => u.counts.post_save_execution_started > 0,
        );
        const executionCompletedUsers = users.filter(
          (u) => u.counts.post_save_execution_completed > 0,
        );
        const overdueShownUsers = users.filter(
          (u) => u.counts.overdue_recovery_shown > 0,
        );
        const captureAndExecutionStartedUsers = captureUsers.filter(
          (u) => u.counts.post_save_execution_started > 0,
        );
        const executionStartedAndCompletedUsers = executionStartedUsers.filter(
          (u) => u.counts.post_save_execution_completed > 0,
        );
        const executionCompletedAndFollowupUsers =
          executionCompletedUsers.filter(
            (u) => u.counts.review_followup_scheduled > 0,
          );
        const overdueShownAndCompletedUsers = overdueShownUsers.filter(
          (u) => u.counts.overdue_recovery_completed > 0,
        );
        return {
          cohortDate,
          users: total,
          activation: {
            firstCaptureSaved,
            firstExecutionCompleted,
            firstFollowupScheduled,
          },
          retention: {
            ...
            studiedTodayRate: pct(
              users.filter((u) => u.studiedToday).length,
              total,
            ),
          },
          loopConversion: {
            captureSavedToExecutionStarted: pct(
              captureAndExecutionStartedUsers.length,
              captureUsers.length,
            ),
            executionStartedToExecutionCompleted: pct(
              executionStartedAndCompletedUsers.length,
              executionStartedUsers.length,
            ),
            executionCompletedToFollowupScheduled: pct(
              executionCompletedAndFollowupUsers.length,
              executionCompletedUsers.length,
            ),
            overdueRecoveryShownToCompleted: pct(
              overdueShownAndCompletedUsers.length,
              overdueShownUsers.length,
            ),
          },
        };
      });
