...
export function projectWeeklyFeasibility(input: { weeklyAvailableMinutes: number; requiredMinimumMinutes: number; requiredMaximumMinutes: number; reasons?: string[] }): FeasibilityProjectionV1 {
  ...
  integer("weekly-available-minutes", input.weeklyAvailableMinutes, 0, MAX_SEVEN_DAY_MINUTES);
  integer("required-minimum-minutes", input.requiredMinimumMinutes, 0, MAX_SEVEN_DAY_MINUTES);
  integer("required-maximum-minutes", input.requiredMaximumMinutes, input.requiredMinimumMinutes, MAX_SEVEN_DAY_MINUTES);
...
export function buildStudyWeekPlan(input: { profile: LearnerConstraintProfileV1; days: DayAvailabilityV1[]; candidates: StudyTaskCandidateV1[]; capacityHistory?: CapacityHistoryDayV1[]; requiredMinimumMinutes: number; requiredMaximumMinutes: number }): StudyWeekPlanV1 {
  ...
  const dayOrdinals = input.days.map((day) => dateOrdinal("study-week-date", day.date));
  if (Math.max(...dayOrdinals) - Math.min(...dayOrdinals) >= 7) throw new Error("week-dates-outside-seven-day-interval");
...
function validatePreviousPlan(plan: StudyDayPlanV1) {
  ...
  if (plan.planGap !== null) {
    ...
    integer("previous-gap-required-plan-minutes", plan.planGap.requiredPlanMinutes, 0, MAX_SEVEN_DAY_MINUTES);
    integer("previous-gap-shortfall-minutes", plan.planGap.shortfallMinutes, 0, MAX_SEVEN_DAY_MINUTES);
