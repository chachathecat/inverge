...
export function createLearningSignal(input) {
  return {
    ...
    completedAt: input.completedAt ?? null,
    evidenceRef: input.evidenceRef ?? null,
    explainableBy: [...(input.explainableBy ?? [])],
  };
}
...
export function buildTodayPlanItemsFromReviewTasks(tasks, now = new Date()) {
  const nowTs = now.getTime();
  return tasks
    .filter((task) => task.dueAt && Date.parse(task.dueAt) <= nowTs)
