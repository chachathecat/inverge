...
export function buildRecurringDeductionSignature(input: {
  ...
  const counterEvidence = input.events.filter(
    (event) =>
      ["D1_REPRODUCED", "D7_TRANSFER_OBSERVED", "TIMED_RECURRENCE_CONFIRMED"].includes(
        event.eventType,
...
export function planDurableEvidence(input: {
  ...
  const proofPassed = durableCommitmentPasses(input.aggregate.caseRecord.subject, input.commitment);
...
export function planFullDayDecision(input: {
  ...
  if (!currentPlan) throw new DurableLearningContractError("invalid_state");
  const plan = { ...currentPlan, decision: input.decision, decisionReason: input.reason };
  const event: Omit<DurableEvidenceEvent, "caseId" | "userId"> = {
    eventId: randomUUID(),
    ...
    transferDistance: null,
    outcome: null,
    payload: { planId: plan.planId, decision: input.decision, reason: input.reason, containsBody: false },
    occurredAt,
  };
  return {
    expectedState: input.aggregate.caseRecord.state,
