...
import {
  ...
  c3rLSourceView,
  c3rLTransferTaskId,
  evaluateC3RLLawClaim,
} from "./c3r-l-engine";
import { createC3RLRepository } from "./c3r-l-repository";
...
const PRIMARY_SURFACE_ID = "server:law-primary-v1" as const;
const FROZEN_CONFIGURATION = Object.freeze({
  answerReviewPolicyVersion: "c3r-l-law-review@1",
  assistancePolicyVersion: "c3r-l-smallest-scaffold@1",
  contentReleaseArtifactVersion: C3R_L_SOURCE.artifactId,
  ledgerSchemaVersion: "c3r-l-ledger@1",
  measurementPolicyVersion: "c3r-l-independent-qualification@1",
  modelVersion: "none:deterministic-structured-law@1",
  promptVersion: "trusted-repair-fixture:appraisal-law@1",
  rubricVersion: "law-exact-applicability-anchor@1",
  schedulerPolicyVersion: "c3r-l-due-review@1",
  validatorVersion: C3R_L_VALIDATOR_ID,
});
const FROZEN_CONFIGURATION_DIGEST = c3rLSha256(FROZEN_CONFIGURATION);
...
function productionDenied() {
  ...
  return process.env.NODE_ENV === "production" && process.env.VERCEL_ENV !== "preview" &&
    process.env.C3R_L_LOCAL_EVIDENCE_MODE !== "true";
}
function isOwner(email: string | null) {
  if (!email) return false;
  const normalized = email.trim().toLowerCase();
  return emails(process.env.ALPHA_ADMIN_EMAILS).includes(normalized) &&
    emails(process.env[C3R_L_OWNER_ALLOWLIST]).includes(normalized);
}

export async function requireC3RLAccess(): Promise<InvergeServerSession & { userId: string }> {
  if (process.env[C3R_L_FEATURE_FLAG] !== "true") throw new C3RLError("feature_disabled");
  if (productionDenied()) throw new C3RLError("production_denied");
  const session = await getServerSessionUser();
  if (!session.isAuthenticated || !session.userId) throw new C3RLError("auth_required");
  if (!isOwner(session.email)) throw new C3RLError("owner_required");
  return { ...session, userId: session.userId };
}

function now(evidenceStep?: string) {
  if (process.env.C3R_L_LOCAL_EVIDENCE_MODE === "true" &&
...
function evidenceRef(issue: number, key: string) {
  return `${C3R_L_RUNTIME_ARTIFACT_REF}#${issue}:${key}`;
}
function requirePass(input: { claim: C3RLLawClaimInput; confirmedAt: string }) {
  const proof = evaluateC3RLLawClaim(input);
  if (!proof.evaluation.verified) throw new C3RLError("invalid_transition");
  return proof;
}
function compareCurrentPlans(left: C3RLPersistedPlan, right: C3RLPersistedPlan) {
  return right.generatedAt.localeCompare(left.generatedAt) || left.planId.localeCompare(right.planId);
}
function comparePlanHistory(left: C3RLPersistedPlan, right: C3RLPersistedPlan) {
  return right.updatedAt.localeCompare(left.updatedAt) ||
    right.generatedAt.localeCompare(left.generatedAt) || left.planId.localeCompare(right.planId);
}

export function createC3RLService(authenticatedUserId: string) {
  const repository = createC3RLRepository(authenticatedUserId);

  async function view(recordId: string | null, asOf = now()): Promise<C3RLView> {
    const resolvedRecordId = recordId ?? await repository.findRecordId(C3R_L_SOURCE);
    ...
    const currentPlan = resolvedRecordId && restored?.record.primary_gap_id && currentReviewPhase
      ...
      : null;
    return {
      source: c3rLSourceView(), restored, dashboard: dashboardProjection, currentPlan,
    ...
    await repository.applyLearningCommand({
      ...
      payload: {
        ...
        itemId: input.action === "complete_d7_transfer"
          ? C3R_L_TRANSFER_TASK.itemId : C3R_L_SOURCE.itemId,
        surfaceId: input.action === "complete_d7_transfer"
          ? C3R_L_TRANSFER_TASK.surfaceId : PRIMARY_SURFACE_ID,
        ...
        ...(C3R_L_PLAN_COMPLETION_ACTIONS.has(input.action) ? {
  ...
  return {
    ...
    async start(input: {
      ...
      await repository.applyLearningCommand({
        ...
        payload: {
          ...
          configurationDigest: FROZEN_CONFIGURATION_DIGEST, occurredAt,
          sourceId: C3R_L_SOURCE.sourceId, problemId: C3R_L_SOURCE.problemId,
          revisionId: C3R_L_SOURCE.revisionId, itemId: C3R_L_SOURCE.itemId,
          artifactId: C3R_L_SOURCE.artifactId,
        },
      });
      return view(input.recordId, occurredAt);
    },
    async commitFeedback(input: {
      ...
      if (!initialAttempt) throw new C3RLError("invalid_transition");
      const gap = c3rLBiggestGap(initialAttempt.body);
      await repository.applyLearningCommand({
      ...
      return {
        view: await view(input.recordId, occurredAt), scaffold: c3rLSourceView().scaffold,
    ...
    async submitRepair(input: {
      ...
      claim: C3RLLawClaimInput; evidenceStep?: string;
    }) {
      const occurredAt = now(input.evidenceStep);
    ...
    async presentD7TransferTask(input: {
      ...
      const occurredAt = now(input.evidenceStep);
      if (input.transferTaskId !== c3rLTransferTaskId(input.recordId)) {
        throw new C3RLError("invalid_transition");
      }
      await repository.applyLearningCommand({
        commandId: input.commandId, expectedVersion: input.expectedVersion,
    ...
    async createPlan(input: {
      ...
      if (result.state !== "PROPOSED") throw new C3RLError("invalid_transition");
    ...
    async decidePlan(input: {
      ...
      blocks: readonly C3RLPlanBlockInput[] | null; evidenceStep?: string;
    }) {
      const asOf = now(input.evidenceStep);
