...
import { ECONOMICS_METHOD, ECONOMICS_PROJECTION_FIELDS, economicsFacts } from "./foundation-economics-facts";

const SUBJECT = "economics_principles", ROLE = "named_owner_authorized_human_subject_reviewer";
const MATRIX = FIVE.deterministicValidatorContractMatrix.economics_principles;
export function economicsValidatorConfiguration(id: typeof MATRIX[number]) {
  return { registryVersion: FIVE.deterministicValidatorRegistry.registryVersion,
    definition: FIVE.deterministicValidatorRegistry.definitions[id as "economics_formula_check" | "economics_graph_check" | "economics_unit_check"],
    canonicalization: "RFC8785", method: ECONOMICS_METHOD };
}
...
export function economicsBodyComponents(evidence: FoundationEvidence, pre: Row) {
  ...
  const projection = evidence.resolve(projectionRefs[0], ECONOMICS_PROJECTION_FIELDS);
  return { subject, subjectRef, manifestRefs, manifest, projectionRefs, projection };
}
export function validateEconomicsPreRelease(evidence: FoundationEvidence, pre: Row, question: Row, choices: readonly unknown[]) {
  const ref = question.reference as Row, bound = economicsBodyComponents(evidence, pre);
  ...
  if (pre.receipt_kind !== "subject_not_applicable_validator" || pre.applicable_version_status !== "not_applicable_verified" ||
    manifest.exam_date !== ECONOMICS_METHOD.examDate || manifest.applicable_version_status !== pre.applicable_version_status) fail();
  requiredIdentifier(manifest.manifest_id); requiredIdentifier(manifest.manifest_version);
  const facts = economicsFacts(evidence, projectionRefs[0], question, choices);
...
export function validateEconomicsReleaseEvidence(ctx: ReleaseContext, release: Row, pre: Row,
  question: Row, choices: readonly unknown[], keyReference: unknown, variant: boolean) {
  const bound = validateEconomicsPreRelease(ctx.evidence, pre, question, choices);
  ...
  // Foundation declares exact sets, not positional arrays. Resolve all THREE
  ...
  for (const validatorId of MATRIX) {
    const definition = economicsValidatorConfiguration(validatorId).definition, actual = values.get(validatorId); if (!actual) fail();
    ...
    same(app.not_applicable_reason_code_or_null, applicable ? null : validatorId === "economics_graph_check"
      ? "no_graph_feature" : "no_dimensioned_value_feature");
    ...
    same(derivation.derivation_method_id, ECONOMICS_METHOD.method); same(derivation.derivation_method_version, ECONOMICS_METHOD.version);
    same(derivation.derivation_configuration_digest, digest(ECONOMICS_METHOD));
    ...
    for (const value of [result, pass]) {
      same(value.validator_configuration_digest, digest(economicsValidatorConfiguration(validatorId)));
