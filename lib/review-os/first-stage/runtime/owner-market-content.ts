...
import { OWNER_ORIGINAL_ADAPTER, OWNER_ORIGINAL_NOTICE, OWNER_ORIGINAL_SCOPE, OWNER_MARKET_IDS as IDS, OWNER_MARKET_VERSION as VERSION, OWNER_MARKET_PACKET_SHA256 as HASH, OWNER_MARKET_SESSION } from "./owner-original-boundary";
import { marketCalculation, type MarketModel } from "./owner-market-calculation";

type Item={id:string;role:string;model:MarketModel;facts:Record<string,string>;prompt:string;choices:{id:ChoiceId;value:string;unit:string;label:string}[];answerChoice:ChoiceId;explanation:string;choiceFeedback:{choice:ChoiceId;text:string}[];calculation:{result:{decimal:string;unit:string}}};
const fail=():never=>{throw new FirstStageKernelError("adapter_mismatch");};
export async function loadOwnerMarketContent(readBytes:()=>Promise<Uint8Array>,assignmentEnabled=false):Promise<PrivateFirstStageCatalog|null>{
 try{
  ...
  const calculations=packet.items.map((item,index)=>{
    ...
    const calculation=marketCalculation(item.model,item.facts);
  ...
  const references=packet.items.map(item=>parseQuestionReference({schemaVersion:"first_stage.owner_original_question_reference.v1",questionId:item.id,questionVersion:VERSION,
    subjectId:"real_estate_principles",examYear:null,examRound:null,questionNumber:null,sessionId:OWNER_MARKET_SESSION,choiceCount:5,
    sourceVersionManifestIds:[`owner-original-${HASH}`],rightsState:"owner_authorized_original",currentnessState:"stated_model_only"}));
  const concept=(index:number)=>({schemaVersion:"first_stage.concept_binding.v1" as const,conceptId:index<2?"stated-linear-rental-equilibrium":"stated-midpoint-demand-elasticity",conceptVersion:"1",subjectId:"real_estate_principles" as const,role:"primary" as const});
  const evidence=(kind:string,value:unknown):ImmutableEvidenceReference=>({schemaVersion:"first_stage.immutable_evidence_reference.v1",evidenceId:`owner-market-${kind}`,evidenceVersion:VERSION,evidenceSha256:digest(value)});
  ...
  const adapter:SubjectAdapterV1={schemaVersion:SUBJECT_ADAPTER_SCHEMA_VERSION,adapterId:OWNER_ORIGINAL_ADAPTER,adapterVersion:"1",subjectId:"real_estate_principles",
    ...
    evaluateSubmission(input){const index=indexFor(input.questionReference),item=packet.items[index],choice=input.submission.selectedChoice;
      ...
      return validateAttemptEvaluation(adapter,input,{schemaVersion:"first_stage.attempt_evaluation.v1",decision,errorCause:null,conceptBindings:[concept(index)],
        ...
        retryDisposition:decision==="correct"?"review_then_retry":"retry_now",reviewAfterMs:decision==="correct"?86400000:0,evaluationPolicyVersion:"owner-market-stated-model-v1",
    ...
    buildIndependentRetry(input){const i=indexFor(input.sourceQuestionReference),c=concept(i);if(i%2||input.priorRetries.length||digest(input.reviewTask.conceptBindings)!==digest([c]))fail();const source=references[i],variant=references[i+1];
      return {schemaVersion:"first_stage.independent_retry_candidate.v1",questionReference:variant,lineageReceipt:{schemaVersion:"first_stage.independent_retry_lineage_receipt.v1",receiptId:`owner-market-lineage-${digest(input.reviewTask.reviewTaskId).slice(0,40)}`,receiptVersion:VERSION,adapterId:adapter.adapterId,adapterVersion:adapter.adapterVersion,subjectId:adapter.subjectId,
  ...
  const catalog:PrivateFirstStageCatalog=Object.freeze({digest:digest({packet:HASH,adapter:adapter.adapterVersion,policy:"owner-market-stated-model-v1"}),registry:createSubjectAdapterRegistry([adapter]),initialReferences:Object.freeze([references[0],references[2]]),
    ...
    curriculumBinding(reference:QuestionReference){const i=indexFor(reference);return {unitId:"re_market",topicId:concept(i).conceptId,title:i<2?"명시 모형: 임대시장 균형과 수요 이동":"명시 모형: 중점법 수요 호탄력성",mappingVersion:VERSION};}});
  authorizeOwnerOriginalCatalog(catalog,[0,2].map((index,priority)=>({candidateId:references[index].questionId,candidateDigest:`sha256:${digest(references[index])}`,familyId:`owner-market-${index<2?"equilibrium":"elasticity"}-v1`,surfaceId:references[index].questionId,bankClass:"LEARNING_PRACTICE",origin:"BANK_STOCK",contentAuthority:"LEARNING_ONLY",rightsStatus:"OWNER_AUTHORIZED_ORIGINAL",sourceStatus:"STATED_MODEL_ONLY",releaseChainComplete:false,unseenEligibilitySnapshotSealed:false,nonSameSurfaceAsSource:false,familyIsolated:false,calibrationState:"UNASSESSED",timedProtocolBound:false,chronology:null,chronologyAuthority:null,availableAt:"2026-09-18T01:39:52.000Z",priority})),assignmentEnabled?[IDS[0],IDS[2]]:[]);
  return catalog;
 }catch{return null;}
}
/** Preserve the prior two-/six-item catalog identity and assignment switches. */
export function composeOwnerMarketCatalog(base:PrivateFirstStageCatalog,market:PrivateFirstStageCatalog,assignmentEnabled:boolean):PrivateFirstStageCatalog {
 ...
 const catalog:PrivateFirstStageCatalog=Object.freeze({digest:digest({base:base.digest,market:market.digest,policy:"owner-original-market-exact-union-v1"}),registry:createSubjectAdapterRegistry([adapter]),initialReferences:Object.freeze([...base.initialReferences,...market.initialReferences]),
