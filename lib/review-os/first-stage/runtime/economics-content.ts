...
export const ECONOMICS_CONTENT_MAX_BYTES = 2 * 1024 * 1024;
export const ECONOMICS_REVIEW_CHECKS = Object.freeze([
...
export type EconomicsContentApproval = Readonly<{
...
function approval(value: EconomicsContentApproval) {
  ...
  if (!["human_reviewed_private", "synthetic_test_only"].includes(value.dataClass) ||
    ...
    digest(value.checks) !== digest(ECONOMICS_REVIEW_CHECKS)) fail();
...
export async function loadEconomicsContent(options: {
  approvals: readonly EconomicsContentApproval[];
  readBytes(): Promise<Uint8Array>;
  expectedDataClass?: EconomicsContentApproval["dataClass"];
}): Promise<PrivateFirstStageCatalog | null> {
  try {
    ...
    if (!bytes.length || bytes.byteLength > ECONOMICS_CONTENT_MAX_BYTES) return null;
    ...
    if (packet.schemaVersion !== "first_stage.economics_private_content.v1" ||
    ...
    const questions = packet.questions.map(value => {
      ...
      if (reference.subjectId !== "economics_principles" ||
    ...
    function boundEvidence(kind: string, value: unknown): ImmutableEvidenceReference {
      return { schemaVersion: "first_stage.immutable_evidence_reference.v1", evidenceId: `economics-${kind}-${digest([approved, value]).slice(0, 40)}`,
        evidenceVersion: approved!.packetVersion, evidenceSha256: digest(value) };
    }
    const adapter: SubjectAdapterV1 = {
      schemaVersion: SUBJECT_ADAPTER_SCHEMA_VERSION, adapterId: "private-economics-reviewed-content",
      ...
      evaluateSubmission(input) {
        ...
        return validateAttemptEvaluation(adapter, input, { schemaVersion: "first_stage.attempt_evaluation.v1", decision,
          ...
          retryDisposition: "review_then_retry", reviewAfterMs: 86_400_000, evaluationPolicyVersion: "private-economics-learning-v1",
      ...
      buildIndependentRetry(input) {
        ...
        return { schemaVersion: "first_stage.independent_retry_candidate.v1", questionReference: candidate.reference,
          lineageReceipt: { schemaVersion: "first_stage.independent_retry_lineage_receipt.v1",
            receiptId: `economics-lineage-${digest([input.reviewTask.reviewTaskId, candidate.reference]).slice(0, 40)}`,
