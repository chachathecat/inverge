...
/** Recompute the bound civil-law facts and EVERY declared assertion. An installed
 * 'pass' label without the exact derivation/result/applicability chain is denied. */
export function validateCivilReleaseEvidence(ctx: ReleaseContext, release: Row, preRelease: Row, keyReference: unknown, variant: boolean) {
  const definitions = FIVE.deterministicValidatorRegistry.definitions;
  const definition = definitions.exam_date_law_snapshot;
  ...
  const binding = { item_id: release.item_id, item_version: release.item_version, subject_id: release.subject_id,
    validator_contract_id: "exam_date_law_snapshot", validator_contract_version: definition.contractVersion };
  ...
  const facts = { authority_ids: ["civil_code"], exam_date: CIVIL_DERIVATION.examDate,
    amendment_chain_receipt_references: preRelease.component_evidence_references,
    applicability_decision_receipt_references: preRelease.component_evidence_references };
  ...
  same(derivation.validator_input_facts_schema_version, "exam_date_law_snapshot.input-facts.v1");
  same(derivation.derivation_method_id, CIVIL_DERIVATION.method); same(derivation.derivation_method_version, CIVIL_DERIVATION.version);
  same(derivation.derivation_configuration_digest, digest(CIVIL_DERIVATION));
  ...
  for (const row of [result, pass]) {
    same(row.validator_configuration_digest, digest(CIVIL_VALIDATOR_CONFIG)); same(row.input_projection_digest, digest(projection));
  ...
  const values: Row = { authority_set_exact: ["civil_code"], exam_date_exact: CIVIL_DERIVATION.examDate,
