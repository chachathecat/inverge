...
// year selector. Post951 adds only real estate from the same preserved source.
// Other subjects remain key-table positions, NOT historical runtime scope.
...
export function profileForQuestion(reference: Record<string, unknown>): OfficialProfile {
  ...
  if (reference.examYear === 2025 && reference.examRound === 36 &&
    ["economics_principles", "real_estate_principles"].includes(String(reference.subjectId))) return ECONOMICS_R3_PROFILE;
  return fail();
}
