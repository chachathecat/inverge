...
export function validatePracticeCalculationRelation(input: {
  ...
  const tail = normalized.slice(relation.sourceIndex + relation.sourceLength, relation.sourceIndex + relation.sourceLength + 24);
  const unitValid = /^\s*(?:원\s*\/\s*년|원\s*\/\s*연|연간\s*원)(?![가-힣A-Za-z0-9])/u.test(tail);
  const rolesValid =
    normalized.includes("총수익") &&
    normalized.includes("운영비") &&
    normalized.includes("순수익");
  ...
  const roundingValid = /반올림\s*(?:없음|하지\s*않음|0(?:자리)?)/u.test(normalized);
  const reasonCodes = [
