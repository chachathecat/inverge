...
export function validateTrustedRepairFixtureEligibility(
  ...
  const manifestWindowValid =
    Number.isFinite(Date.parse(manifest.validFrom)) &&
    Number.isFinite(Date.parse(manifest.validUntil)) &&
    Date.parse(manifest.validFrom) <= Date.parse(decision.rightsEvaluatedAt) &&
...
export const TRUSTED_REPAIR_GOLD_CANDIDATES: readonly TrustedRepairGoldCandidate[] =
  GOLD_FAMILIES.flatMap(([familyId, subject, label, anchorId]) =>
    ([1, 2] as const).map((sampleIndex) => ({
      ...
      answerSample:
        sampleIndex === 1
          ? "연간 총수익 - 연간 운영비 = 연간 순수익: 120,000,000 - 20,000,000 = 100,000,000원/년, 양수이며 반올림 없음."
