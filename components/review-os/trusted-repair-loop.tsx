...
type DurablePendingCommand = PendingCommand & {
  ownerScope: string;
};
...
function isPendingCommand(
  value: unknown,
  ownerScope: string,
): value is DurablePendingCommand {
  ...
  return (
    ...
    UUID_PATTERN.test(record.ownerScope) &&
    record.ownerScope === ownerScope
  );
}

function readDurablePendingCommand(ownerScope: string) {
  try {
    const encoded = window.sessionStorage.getItem(PENDING_COMMAND_STORAGE_KEY);
    if (!encoded) return null;
    const parsed: unknown = JSON.parse(encoded);
    if (isPendingCommand(parsed, ownerScope)) return parsed;
...
function writeDurablePendingCommand(
  ...
  try {
    window.sessionStorage.setItem(
      ...
      JSON.stringify({ ...command, ownerScope } satisfies DurablePendingCommand),
    );
  } catch {
...
export function TrustedRepairLoop({ ownerScope }: { ownerScope: string }) {
  ...
  async function command(action: ApiAction, fields: Record<string, unknown> = {}) {
    ...
    try {
      ...
      if (action === "start") {
        writeDurablePendingCommand(pendingCommandRef.current, ownerScope);
      }
  ...
  function structuredClaim() {
    ...
    const integer = (value: string) =>
      /^-?\d+$/u.test(value.trim()) && Number.isSafeInteger(Number(value.trim()))
        ? Number(value.trim())
        : null;
    const gross = integer(grossIncome);
    const expense = integer(operatingExpense);
    const result = integer(resultValue);
    if (!confirmation || gross === null || expense === null || result === null) {
      return null;
    }
    return {
      ...
      grossIncome: { value: gross, unit: confirmation.unit },
      operatingExpense: { value: expense, unit: confirmation.unit },
      operator: confirmation.operator,
      operandOrder: confirmation.operandOrder,
      result: { value: result, unit: confirmation.unit },
      ...
      >
        ...
        {!view ? (
          ...
          <div className="space-y-5">
            ...
            {view.session.state === "repair_submitted" && view.structuredConfirmation ? (
              <fieldset className="space-y-3 rounded-lg border border-[var(--color-border-focus)] p-4">
                <legend className="px-1 v3-type-label-strong">직접 확인하는 계산관계</legend>
                <p className="text-sm text-[var(--color-text-secondary)]">
                  자유서술은 후보 근거일 뿐입니다. 아래 닫힌 필드를 모두 직접 확인해야 같은 세션 검증이 가능합니다.
                </p>
                <label className="block space-y-1">
                  <span className="text-sm font-medium">총수익 · 원/년</span>
                  <input aria-label="총수익 원/년" inputMode="numeric" value={grossIncome} onChange={(event) => setGrossIncome(event.target.value)} className="min-h-11 w-full rounded-md border bg-transparent px-3" />
                </label>
                <label className="block space-y-1">
                  <span className="text-sm font-medium">운영비 · 원/년</span>
                  <input aria-label="운영비 원/년" inputMode="numeric" value={operatingExpense} onChange={(event) => setOperatingExpense(event.target.value)} className="min-h-11 w-full rounded-md border bg-transparent px-3" />
                </label>
                <label className="block space-y-1">
                  <span className="text-sm font-medium">결과 · 원/년</span>
                  <input aria-label="결과 원/년" inputMode="numeric" value={resultValue} onChange={(event) => setResultValue(event.target.value)} className="min-h-11 w-full rounded-md border bg-transparent px-3" />
                </label>
                <dl className="grid gap-1 text-sm text-[var(--color-text-secondary)]">
                  <div><dt className="inline font-medium">연산:</dt> <dd className="inline">총수익 − 운영비</dd></div>
