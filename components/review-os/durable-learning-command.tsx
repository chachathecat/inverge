...
const REQUIRED_COMMITMENT_FIELDS = {
  appraisal_practical: ["anchorId", "grossIncome", "operatingExpense", "result", "operator", "unit", "sign", "rounding"],
  appraisal_theory: ["anchorId", "targetScopeId", "requiredPredicate", "polarity", "forbiddenPredicateAsserted"],
  appraisal_law: ["anchorId", "sourceId", "sourceVersionId", "lawAnchorId", "lawAnchorVersionId", "exactLocator", "applicableAsOf", "blockerCount", "currentness"],
...
function commitmentFieldsComplete(
  subject: DurableLearningView["case"]["subject"],
  fields: Fields,
) {
  return REQUIRED_COMMITMENT_FIELDS[subject].every((field) => Boolean(fields[field]?.trim()));
}
...
function commitmentFor(subject: DurableLearningView["case"]["subject"], fields: Fields) {
  if (subject === "appraisal_practical") {
    return {
      ...
      grossIncome: explicitNumber(fields.grossIncome),
      operatingExpense: explicitNumber(fields.operatingExpense),
...
export function DurableLearningCommand({ ownerScope }: { ownerScope: string }) {
  ...
  const primary = !view
    ...
    : view.case.nextAction === "SUBMIT_INDEPENDENT_ATTEMPT"
        ? { label: "독립 시도 제출 및 검증", disabled: body.trim().length === 0 || !commitmentFieldsComplete(view.case.subject, fields), run: () => post("record_evidence", { body, commitment: commitmentFor(view.case.subject, fields) }) }
