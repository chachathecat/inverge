...
function commitmentFor(subject: DurableLearningView["case"]["subject"], fields: Fields) {
  if (subject === "appraisal_practical") {
    return {
      kind: "PRACTICE_CALCULATION",
      anchorId: fields.anchorId,
      grossIncome: Number(fields.grossIncome),
      operatingExpense: Number(fields.operatingExpense),
  ...
  if (subject === "appraisal_theory") {
    return {
      ...
      anchorId: fields.anchorId,
      targetScopeId: fields.targetScopeId,
      requiredPredicate: fields.requiredPredicate,
      forbiddenPredicateAsserted: fields.forbiddenPredicateAsserted === "true",
  ...
  return {
    ...
    lawAnchorId: fields.lawAnchorId,
    lawAnchorVersionId: fields.lawAnchorVersionId,
    exactLocator: fields.exactLocator,
    applicableAsOf: fields.applicableAsOf,
    currentness: fields.currentness,
    blockerCount: Number(fields.blockerCount),
  };
}

function SubjectCommitmentFields({
  subject,
  fields,
  setFields,
}: {
  ...
  if (subject === "appraisal_practical") {
    return (
      <div className="grid gap-3 sm:grid-cols-2">
        {field("anchorId", "계산 앵커 ID")}
        {field("grossIncome", "총수익", "numeric")}
        {field("operatingExpense", "운영비", "numeric")}
  ...
  if (subject === "appraisal_theory") {
    return (
      <div className="grid gap-3 sm:grid-cols-2">
        {field("anchorId", "이론 앵커 ID")}
        {field("targetScopeId", "정확한 목표 범위 ID")}
        {field("requiredPredicate", "필수 술어 ID")}
        <label className="space-y-1"><span className="text-sm font-medium">필수 술어 극성</span><select aria-label="필수 술어 극성" value={fields.polarity ?? ""} onChange={(event) => setFields({ ...fields, polarity: event.target.value })} className={INPUT_CLASS}><option value="">선택</option><option value="POSITIVE">POSITIVE</option><option value="NEGATIVE">NEGATIVE</option></select></label>
        <label className="space-y-1"><span className="text-sm font-medium">금지 술어를 주장했는가</span><select aria-label="금지 술어 주장 여부" value={fields.forbiddenPredicateAsserted ?? ""} onChange={(event) => setFields({ ...fields, forbiddenPredicateAsserted: event.target.value })} className={INPUT_CLASS}><option value="">선택</option><option value="false">아니요</option><option value="true">예</option></select></label>
      </div>
    );
  }
  return (
    <div className="grid gap-3 sm:grid-cols-2">
      {field("anchorId", "복구 앵커 ID")}
      {field("sourceId", "법령 출처 ID")}
      {field("sourceVersionId", "법령 출처 버전 ID")}
      {field("lawAnchorId", "법령 앵커 ID")}
      {field("lawAnchorVersionId", "법령 앵커 버전 ID")}
      {field("exactLocator", "정확 위치")}
      {field("applicableAsOf", "적용 기준일")}
      {field("blockerCount", "열린 차단 근거 수", "numeric")}
      <label className="space-y-1"><span className="text-sm font-medium">현재성</span><select aria-label="현재성" value={fields.currentness ?? ""} onChange={(event) => setFields({ ...fields, currentness: event.target.value })} className={INPUT_CLASS}><option value="">선택</option><option value="APPLICABLE_CURRENT">APPLICABLE_CURRENT</option><option value="STALE">STALE</option><option value="UNKNOWN">UNKNOWN</option></select></label>
    </div>
  );
}
...
export function DurableLearningCommand({ ownerScope }: { ownerScope: string }) {
  ...
  function accept(next: DurableLearningView) {
    setView(next);
    setBody("");
    setFields({});
  ...
  async function post(action: string, fields: Record<string, unknown> = {}) {
    ...
    try {
      ...
      if (payload.view) accept(payload.view as DurableLearningView);
      return payload;
    } catch (caught) {
  ...
  const primary = !view
    ...
    : view.case.nextAction === "PREPARE_INDEPENDENT_ATTEMPT"
      ...
      : view.case.nextAction === "SUBMIT_INDEPENDENT_ATTEMPT"
        ? { label: "독립 시도 제출 및 검증", disabled: body.trim().length === 0, run: () => post("record_evidence", { body, commitment: commitmentFor(view.case.subject, fields) }) }
      ...
      <section className="space-y-5 rounded-[var(--v3-radius-card)] border border-[var(--color-border-default)] bg-[var(--color-background-elevated)] p-4 shadow-sm sm:p-6" aria-labelledby="durable-state-heading">
        ...
        {view?.attempt ? (
          <div className="space-y-4">
            ...
            <fieldset className="space-y-3 rounded-lg border border-[var(--color-border-focus)] p-4"><legend className="px-1 v3-type-label-strong">직접 확인하는 과목별 증명</legend><p className="text-sm text-[var(--color-text-secondary)]">자유서술이나 토큰 존재만으로 검증되지 않습니다. 답을 보지 않고 닫힌 증명 필드를 직접 채우세요.</p><SubjectCommitmentFields subject={view.case.subject} fields={fields} setFields={setFields} /></fieldset>
          </div>
        ) : null}
        ...
        <details className="rounded-[var(--v3-radius-card)] border border-[var(--color-border-default)] bg-[var(--color-background-elevated)] p-4">
          ...
          <div className="mt-4 space-y-5">
            ...
            <Button type="button" variant="outline" disabled={busy} onClick={() => {
              const minutes = Number(fixedMinutes);
              if (minutes > 0 && !fixedCommitmentId.current) fixedCommitmentId.current = crypto.randomUUID();
              void post("build_plan", { availableMinutes: Number(availableMinutes), recoveryMode, fixedCommitments: minutes > 0 ? [{ commitmentId: fixedCommitmentId.current, label: "MANUAL_COMMITMENT", minutes }] : [] });
            }}>근거 우선 계획 만들기</Button>
            {view.latestPlan ? (
