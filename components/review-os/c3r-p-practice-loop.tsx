...
function practiceClaim(resultValue = 100_000_000): C3RPPracticeClaimInput {
  return {
    ...
    result: { value: resultValue, unit: "KRW_PER_YEAR" },
    sign: "POSITIVE",
...
export function C3RPPracticeLoop({
  ...
  const [repairBody, setRepairBody] = useState(
    "120,000,000원/년 - 20,000,000원/년 = 100,000,000원/년, 양수, 반올림 없음",
  );
  ...
  async function review(
    ...
    await request({
      ...
      attemptId: id(),
      attemptBody:
        action === "record_later_failure"
          ? "120,000,000 - 20,000,000 = 90,000,000원/년"
          : repairBody,
      claim: practiceClaim(action === "record_later_failure" ? 90_000_000 : 100_000_000),
      surfaceId: action === "complete_d7_transfer" ? "browser-transfer" : "browser-primary",
      evidenceStep,
    });
  }
  ...
  async function decidePlan(decision: "ACCEPT" | "EDIT" | "REJECT") {
    ...
    setView((current) => current && current.currentPlan
      ? {
          ...current,
          currentPlan: {
            ...current.currentPlan,
            recordVersion: current.currentPlan.recordVersion + 1,
            state: decision === "ACCEPT" ? "ACCEPTED" : decision === "EDIT" ? "EDITED" : "REJECTED",
            blocks: blocks ?? current.currentPlan.blocks,
          },
        }
      : current);
  }
      ...
      <section className="rounded-2xl border border-[var(--color-border-default)] bg-white p-5 shadow-sm">
        ...
        {record?.state === "FEEDBACK_COMMITTED" ? (
          <div className="mt-5 grid gap-3">
            ...
            <label className="grid gap-1 text-sm font-medium">
              직접 재계산·재작성
              <textarea className="min-h-28 rounded-xl border p-3 font-normal" value={repairBody} onChange={(event) => setRepairBody(event.target.value)} />
            </label>
        ...
        {record?.state === "CLOSED" ? (
          <button disabled={pending} onClick={() => void review("record_later_failure")} className="mt-5 w-full rounded-xl border border-amber-500 px-4 py-3 font-semibold text-amber-800 disabled:opacity-50">
            후속 독립 실패를 기록해 간극 다시 열기
          </button>
        ) : null}
