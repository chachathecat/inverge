...
import {
  C3R_T_ANCHOR_ID,
  C3R_T_ANCHOR_VERSION_ID,
  C3R_T_TARGET_SCOPE_ID,
  c3rTCurrentQueueItem,
...
const apiPath = "/api/review-os/c3r-t";
function id() { return crypto.randomUUID(); }

export function C3RTTheoryLoop({ initialRecordId }: { initialRecordId: string | null }) {
  const [view, setView] = useState<C3RTView | null>(null);
  ...
  function claim(forceFailure = false) {
    const predicates: Array<{ predicateId: string; polarity: "ASSERTED" | "NEGATED" }> = [{
      predicateId: "converts_expected_income_to_value",
      polarity: requiredPolarity,
    }];
    const resolvedForbidden = forceFailure ? "ASSERTED" : forbiddenPolarity;
    if (resolvedForbidden !== "OMIT") {
      predicates.push({ predicateId: "uses_only_historical_cost", polarity: resolvedForbidden });
    }
    return {
      sourceRevisionId: view?.source.revisionId,
      anchorId: C3R_T_ANCHOR_ID,
      anchorVersionId: C3R_T_ANCHOR_VERSION_ID,
      targetScopeId: C3R_T_TARGET_SCOPE_ID,
      clauses: [{
        clauseIndex: 1,
        scopeResolution,
  ...
  async function start() {
    ...
    if (data.ok) window.history.replaceState(null, "", `/app/c3r-t?recordId=${recordId}`);
  }
  ...
  async function decidePlan(decision: "ACCEPT" | "EDIT" | "REJECT") {
    ...
    const blocks: readonly C3RTPlanBlockInput[] | null = decision === "EDIT"
  ...
  async function exportData() {
    ...
    anchor.download = "dabangil-c3r-t-theory-export.json";
    anchor.click();
  ...
  async function deleteData() {
    ...
    if (data.result?.status === "deleted") {
      ...
      window.history.replaceState(null, "", "/app/c3r-t");
    }
  }

  if (!view) return <main className="mx-auto max-w-3xl p-6" data-testid="c3r-t-loading">이론 학습 상태를 불러오는 중입니다.</main>;
