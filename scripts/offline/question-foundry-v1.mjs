...
async function run(options) {
  ...
  if (options.action === "validate-blueprint") {
    return validateQuestionBlueprint(input.blueprint ?? input);
  }
  if (options.action === "validate-answer-specification") {
    return validateAnswerSpecification(input.answerSpecification, input.blueprint);
  }
  if (options.action === "validate-batch") return validateCandidateBatch(input.batch ?? input);
  if (options.action === "validate-sources") {
    return validateTrustedSourceBindings(input.bindings, input.registry, input.purposes);
  }
