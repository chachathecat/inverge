...
function qualifiedStatementObjectKinds(occurrence) {
  ...
  if (
    (isKeyword(command, "create") ||
      isKeyword(command, "alter") ||
      isKeyword(command, "drop")) &&
    isKeyword(tokens[start + 1], "policy") &&
    depth === 0
  ) {
    const onIndex = topLevelKeywordIndex(tokens, start + 2, end, "on");
...
function deriveQualifiedIndexOperationIdentifiers(sql) {
  ...
  for (let index = 0; index < tokens.length; index += 1) {
    ...
    } else if (isKeyword(tokens[cursor], "drop") && isKeyword(tokens[cursor + 1], "index")) {
      cursor += 2;
      if (isKeyword(tokens[cursor], "concurrently")) cursor += 1;
      if (isKeyword(tokens[cursor], "if") && isKeyword(tokens[cursor + 1], "exists")) {
        cursor += 2;
    ...
    const reference = qualifiedIdentifierAt(tokens, cursor);
    if (reference) identifiers.add(qualifiedSemanticKey(reference));
  }
  return identifiers;
}
...
function occurrenceAt(tokens, index) {
  const reference = qualifiedIdentifierAt(tokens, index);
  return reference
    ? {
        ...reference,
...
function deriveStatementOrderedDatabaseObjectReferences(
  ...
  const indexTargets = deriveQualifiedIndexOperationIdentifiers(sql);
  const state = new Map(
  ...
  for (const scope of executableStatementScopeRecords(sql)) {
    ...
    for (let index = 0; index < tokens.length; index += 1) {
      const occurrence = occurrenceAt(tokens, index);
      if (!occurrence) continue;
      if (
        !producerTargets.has(index) &&
        !nonReportableTargets.has(index) &&
        !indexTargets.has(qualifiedSemanticKey(occurrence)) &&
    ...
    createdExtensionNames: [
      ...new Set(
        derived.flatMap((entry) =>
          entry.createdExtensions.map((item) => item.name),
        ),
      ),
    ].sort(),
