...
export const ORACLE_POSTGRES_SOURCE_TAG = "REL_15_8";
export const ORACLE_POSTGRES_SOURCE_COMMIT =
  "3cc5e51ab4507b33acc82684b7d79ac43b8a6b5d";
export const ORACLE_PG15_8_REVOKE_FIXTURE_ID =
  "membership_revoke_nonexistent_granted_by_clause_ignored_pg15_8";
export const ORACLE_PG15_8_REVOKE_MEMBERSHIP_DELTA_SHA256 =
  "67d4c452ab475b5a882aadad3b7cb9b9d1921bb64695ac5389edcfd721deb6ae";
export const ORACLE_PG15_8_REVOKE_DELTA_SHA256 =
  "d967a26127a9d53434542ccecc55f3f02a117fcfa413766f7e6fa3922fe63b96";
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE roleid = 'oracle_grantor'::regrole
      AND member = 'oracle_direct'::regrole
      AND grantor = 'postgres'::regrole
      AND admin_option IS FALSE
  ) THEN
    RAISE EXCEPTION USING ERRCODE = 'P0001', MESSAGE = 'required membership precondition is absent';
  END IF;
END;
$oracle$;
...
export function assertPg15_8RevokeMembershipDelta({
  collectionCounts,
  collectionDigests,
  sha256: deltaSha256,
}) {
  assertExactKeys(
    ...
    "PostgreSQL 15.8 REVOKE delta counts",
  );
  assertExactKeys(
    collectionDigests,
    ORACLE_SNAPSHOT_COLLECTIONS,
    "PostgreSQL 15.8 REVOKE delta digests",
  );
  for (const collection of ORACLE_SNAPSHOT_COLLECTIONS) {
    const expectedCounts = collection === "memberships"
      ? { added: 0, changed: 0, removed: 1 }
      : { added: 0, changed: 0, removed: 0 };
    const expectedDigest = collection === "memberships"
      ? ORACLE_PG15_8_REVOKE_MEMBERSHIP_DELTA_SHA256
      : EMPTY_SEMANTIC_DELTA_SHA256;
    if (
      canonicalJson(collectionCounts[collection]) !== canonicalJson(expectedCounts) ||
      collectionDigests[collection] !== expectedDigest
    ) {
      throw new Error(
        "PostgreSQL 15.8 REVOKE did not prove the exact membership-only removal.",
      );
    }
  }
  if (deltaSha256 !== ORACLE_PG15_8_REVOKE_DELTA_SHA256) {
    throw new Error("PostgreSQL 15.8 REVOKE exact semantic delta digest is invalid.");
  }
  return true;
}
...
function executeOracleFixtures(containerName) {
  ...
  for (const step of oracleFixtureSteps()) {
    ...
    if (step.id === ORACLE_PG15_8_REVOKE_FIXTURE_ID) {
      assertPg15_8RevokeMembershipDelta(delta);
    }
    let actual;
    let sqlstate = null;
    ...
    if (step.expectation === "rejection") {
      ...
      if (
        step.id === ORACLE_PG15_8_REVOKE_FIXTURE_ID &&
        result.stderr.trim() !== ""
      ) {
        throw new Error(
          "PostgreSQL 15.8 REVOKE emitted an unexpected SQLSTATE diagnostic.",
        );
      }
