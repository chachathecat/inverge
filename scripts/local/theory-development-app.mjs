...
async function serve(production=false, controlledPractice=false, paidPractice=false, controlledLaw=false, captureOnly=false, originalPractice=false){
 const c=await config();if(c.userId!==userId)fail("isolated_config_mismatch");
 const lawAccount=controlledLaw?await (await import("./law-development-account.mjs")).readLawTestAccount():null;
 ...
 if(captureOnly) Object.assign(env,{INVERGE_OWNER_PC_THEORY_ENABLED:"false",INVERGE_OWNER_PC_THEORY_DEVELOPMENT_ENABLED:"false",...(process.env.INVERGE_LOCAL_OCR_ENABLED==="true"?{INVERGE_LOCAL_OCR_ENABLED:"true"}:{})});
 ...
 if(controlledLaw) Object.assign(env,{ALPHA_ADMIN_EMAILS:lawAccount.email,INVERGE_OWNER_PC_THEORY_ENABLED:"false",INVERGE_OWNER_PC_THEORY_DEVELOPMENT_ENABLED:"false",WCV_C2R_C_T_THEORY_ENABLED:"false",WCV_C2R_C_L_LAW_ENABLED:"true",WCV_C2R_C_L_OWNER_EMAILS:lawAccount.email,GEMINI_API_KEY:"synthetic-law-no-live-key",GEMINI_MODEL:"gemini-2.5-flash",INVERGE_LAW_CONTROLLED_PROVIDER:"true",NODE_OPTIONS:`--import=${new URL("./law-development-provider.mjs",import.meta.url).href}`});
...
const command=process.argv[2];(command==="prepare"?prepare():command==="serve"?serve():command==="serve-production"?serve(true):command==="serve-original-practice"?serve(false,false,false,false,false,true):command==="serve-capture"?serve(false,false,false,false,true):command==="serve-law-controlled"?serve(true,false,false,true):command==="serve-practice-controlled"?serve(true,true):command==="serve-practice-paid"?serve(true,false,true):command==="authorize-practice-development"?authorizePracticeCalls():command==="authorize-additional-practice-call"?authorizePracticeCalls(true):command==="authorize-development"?authorizeDevelopment():command==="authorize-additional-call"?authorizeAdditionalCall():Promise.reject(Error("invalid_isolated_command"))).catch(error=>{console.error(/^[a-z0-9_]+$/.test(error.message)?error.message:"isolated_runtime_failed_no_secret_output");process.exitCode=1;});
