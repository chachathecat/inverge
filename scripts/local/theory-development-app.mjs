...
import {authorizeTheoryDevelopment,readTheoryDevelopmentApproval,readTheoryBudget,validateTheorySettings} from "../../lib/owner-study/owner-pc-theory-budget.mjs";
...
async function serve(){
 ...
 const child=spawn(process.execPath,["node_modules/next/dist/bin/next","dev","--hostname","127.0.0.1","--port","3884"],{cwd:root,env,stdio:"inherit",windowsHide:true});child.on("exit",code=>{gateway.close();process.exitCode=code??1;});
}
const command=process.argv[2];(command==="prepare"?prepare():command==="serve"?serve():command==="authorize-development"?authorizeDevelopment():Promise.reject(Error("invalid_isolated_command"))).catch(error=>{console.error(/^[a-z0-9_]+$/.test(error.message)?error.message:"isolated_runtime_failed_no_secret_output");process.exitCode=1;});
