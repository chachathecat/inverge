...
export default async function ReviewOsCapturePage({ searchParams }: PageProps) {
  ...
  const { session, access, profile } = await getReviewOsServerContext(buildReviewOsReturnTo("/app/capture", modeParam));
  ...
  const initialSubject = normalizeSubjectForMode(query?.subject ?? (mode === "second" ? ownerPcSecondInitialSubject() : undefined), mode);
  const rewriteDetail =
