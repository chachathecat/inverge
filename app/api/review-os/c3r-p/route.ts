...
function requiredInteger(value: unknown, minimum = 0) {
  ...
  try {
    ...
    if (action === "start") {
      const row = c3rPExactObject(raw, [
        ...
        "prediction", "confidence", "surfaceId", "evidenceStep",
      ]);
      ...
      const view = await service.start({
        ...
        surfaceId: c3rPRequiredText(row.surfaceId, 120),
    ...
    if (action === "submit_repair") {
      const row = c3rPExactObject(raw, [
        ...
        "attemptBody", "claim", "evidenceStep",
      ]);
    ...
    if ([
      ...
      const row = c3rPExactObject(raw, [
        "action", "commandId", "recordId", "expectedVersion", "attemptId",
        "attemptBody", "claim", "surfaceId", "evidenceStep",
      ]);
    ...
    if (action === "create_plan") {
      ...
      const view = await service.createPlan({
        ...
        availableMinutes: requiredInteger(row.availableMinutes, 1),
        evidenceStep: optionalEvidenceStep(row.evidenceStep),
      });
      return response({ ok: true, view });
    }
