...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (session.userId) await reviewOsRepository.logUsageEvent(session.userId, "capture_ocr_success", "capture_session", null, { mode, imageCount: imageFiles.length });
    ...
    return NextResponse.json({ ok: true, text: rawOcrText, ...extraction });
  } catch (error) {
