...
export default async function PrivateAccountingPracticePage() {
  const owner = await requirePrivateFirstStageOwner();
  if (!owner) notFound();
  // Only subject choice metadata crosses RSC. No content, key, explanation or approval.
  return <ReviewOsAppShell email={owner.email}><FirstStagePrivatePractice subject="accounting" /></ReviewOsAppShell>;
}
